...
The following definition is adapted from~\cite{ButlerRingel}. We note that it is customary to consider non-finite dimensional gentle algebras in the literature, see e.g.~\cite{Schoer}.
...
\begin{example}
Considering our choice of conventions, we represent the string ${\rho = \gamma\beta^{-1}\alpha^{-1}\gamma\beta^{-1}\alpha^{-1}\gamma\delta}$ as follows:
\[
...
\begin{notation}
To avoid distinguishing between finite, infinite and bi-infinite words, we denote strings as products~$\rho = \prod_{i < \ell < j} \alpha_\ell^{\epsilon_\ell}$ where~$i < j \in \Z \cup \{\pm \infty\}$, $\epsilon_\ell\in\{\pm 1\}$ and~$\alpha_\ell \in Q_1$ for all~$i < \ell < j$.
\end{notation}
...
A simplicial complex is \defn{thin} if each of its codimension~$1$ face is contained in exaclty two facets.
