...
We apply Remark~\ref{rem:propertiesSurface} to understand the corresponding surfaces.
The corresponding perfect matchings (see Remark~\ref{rem:propertiesSurface}\,\eqref{item:boundary}) look like~\raisebox{-.1cm}{\includegraphics[scale=.3]{reversedPathsPM}}.
...
\enlargethispage{.2cm}
We apply Remark~\ref{rem:propertiesSurface} to understand the corresponding surfaces.
The corresponding perfect matchings (see Remark~\ref{rem:propertiesSurface}\,\eqref{item:boundary}) look like~\raisebox{-.1cm}{\includegraphics[scale=.3]{doublePathsPModd}} when~$|Q_0|$ is odd and~\raisebox{-.1cm}{\includegraphics[scale=.3]{doublePathsPMeven}} when~$|Q_0|$ is even.
Moreover, $\bar Q$ and~$\bar Q\koszul$ have no infinite straight walks.
...
We invite the reader to check that~$\curveof(v_\peak) = \vprevious{\edgeof(v)}$ (resp.~$\curveof(v_\deep) = \vnext{\edgeof(v)}$), where~$\edgeof(v)$ denotes the edge of~$\dissection_{\bar Q}$ corresponding to~$v \in Q_0$ (see Remark~\ref{rem:propertiesSurface}\,\eqref{item:edges}).
...
An arrow~$\alpha$ in~$\bar Q\blossom$ corresponds to an angle at a point~$v$ between two edges of~$\dissection$ (resp.~at a point~$v\dual$ between two edges of~$\dissection\dual$).
...
Let~$\bar Q = (Q,I)$ be a locally gentle bound quiver, and let~$(\b{e}_v)_{v \in Q_0}$ denote the standard basis of~$\R^{Q_0}$.
...
According to Remark~\ref{rem:propertiesSurface}\,\eqref{item:edges}, each vertex~$p$ of~$Q$ corresponds to an edge~$\edgeof(p)$ in the dissection~$\dissection_{\bar Q}$ and  to an edge~$\dualedgeof(p) = \edgeof(p)\dual$ in the dual dissection~${\dissection\dual\!\!_{\bar Q}}$.
...
\begin{enumerate}[(i)]
\item The number of times that the $\dissection$-accordion~$\curveof(\omega)$ positively crosses the edge~$\edgeof(p)$ of~$\dissection$ minus the number of times it negatively crosses the edge~$\edgeof(p)$ of~$\dissection$. Here, the crossing is positive (resp.~negative) if the two angles of~$\dissection$ crossed by~$\curveof(\alpha)$ just before and after~$\edgeof(p)$ form a~$\SSS$ (resp.~a~$\ZZZ$).
...
Denote by~$X_\deep \eqdef \set{\curveof(v_\peak)}{v \in Q_0}$ the deep facet of the non-crossing complex obtained by slightly rotating~$\dissection_{\bar Q}$ counterclockwise as described in Example~\ref{exm:peakDeepFacets}.
For a walk~$\omega$ of~$\bar Q$ and a vertex~$v \in Q_0$, the $v$-th entry of the $\b{d}$-vector~$\dvector{\omega}$ is either~$-1$ if~$\omega = v_\peak$ or the number of crossings of the curve~$\gamma(\omega)$ with the edge~$\curveof(v_\peak)$ of~$X_\deep$ corresponding to~$v$.
\end{proposition}
