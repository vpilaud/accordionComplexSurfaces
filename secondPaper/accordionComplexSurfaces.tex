...
%We call \defn{negative simple string} a formal word of length zero of the form $-a$, where $a$ is any vertex of $Q_0$. We denote by
%\(
%\strings^\pm_{\ge -1}(\bar Q) \eqdef \strings^\pm(\bar Q) \sqcup \set{-a}{a \in Q_0}
%\)
%the set of \defn{almost positive unoriented strings}.
%Note that~$\varepsilon_a$ and~$-a$ are different almost positive strings.
%\end{enumerate}
\end{definition}
...
We invite the reader to check that~$\curveof(a_\peak) = \vprevious{\edgeof(a)}$ (resp.~$\curveof(a_\deep) = \vnext{\edgeof(a)}$), where~$\edgeof(a)$ denotes the edge of~$\dissection$ corresponding to~$a \in Q_0$ (see Remark~\ref{rem:propertiesSurface}).
...
An arrow~$\alpha$ in~$\bar Q\blossom$ corresponds to an angle at a point~$v$ between two arcs of~$\dissection$ (resp.~at a point~$f\dual$,~$v\in f$, between two arcs of~$\dissection\dual$).
...
Let~$\bar Q = (Q,I)$ be a locally gentle bound quiver, and let~$(\b{e}_a)_{a \in Q_0}$ be the standard basis of~$\R^{Q_0}$.
...
According to Remark~\ref{rem:propertiesSurface}\,(\ref{item:arcs}), each vertex~$a$ of~$Q$ corresponds to an edge~$\edgeof(a)$ in the dissection~$\dissection_{\bar Q}$ and  to an edge~$\dualedgeof(a) = \edgeof(a)\dual$ in the dual dissection~${\dissection\dual\!\!_{\bar Q}}$.
...
For a walk~$\omega$ of~$\bar Q$ and a vertex~$a \in Q_0$, the $a$-th entry of the $\b{d}$-vector~$\dvector{\omega}$ is the number of crossings of the curve~$\gamma(\omega)$ with the edge of~$X_\deep$ corresponding to~$a$.
\end{proposition}
