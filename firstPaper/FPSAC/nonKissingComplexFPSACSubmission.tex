...
\documentclass[submission]{FPSAC2018Submission}
...
\author{Yann Palu\addressmark{1}, Vincent Pilaud\addressmark{2} \and Pierre-Guy Plamondon\addressmark{3}\thanks{\href{mailto:yann.palu@u-picardie.fr}{yann.palu@u-picardie.fr}, \href{mailto:}{vincent.pilaud@lix.polytechnique.fr}, \href{mailto:}{pierre-guy.plamondon@math.u-psud.fr}. The three authors were partially supported by the French ANR grant SC3A~(15\,CE40\,0004\,01).}}
...
Ce complexe offre un modèle combinatoire pour la théorie du $\tau$-basculement à support des algèbres aimables et il généralise et unifie les cas particuliers définis à partir de sous-ensembles de la grille ou de dissections de polygones (contenant notamment le cas de l'associaèdre classique).
Dans ce résumé étendu, nous présentons des propriétés combinatoires et géométriques des complexes platoniques finis : nous montrons que leurs graphes de flips sont les diagrammes de Hasse de treillis congruence-uniformes, et qu'ils peuvent être réalisés par des polytopes convexes.
}
...
\addbibresource{nonKissingComplexFPSACSubmission.bib}
...
As already observed in~\cite[Sect.~10]{McConville}, when the shape is a ribbon, the non-kissing complex is an associahedron, and the non-kissing lattice is a type~$A$ Cambrian lattice of N.~Reading~\cite{Reading-CambrianLattices}.
...
We first define in Sec.~\ref{sec:nonkissingcomplex} the non-kissing complex of a gentle bound quiver and show that this complex is a pseudomanifold (meaning in particular that there is a well-defined notion of flips in non-kissing facets).
In Sec.~\ref{sec:nonkissinglattice}, we show that the graph of increasing flips is the Hasse diagram of a congruence-uniform lattice and describe its canonical join complex.
Finally, Sec.~\ref{sec:gentleAssociahedra} is devoted to the geometry of finite non-kissing complexes: we construct their $\b{g}$-vector fans and show that these fans are normal fans of convex polytopes.
...
The \defn{blossoming quiver} of~${\bar Q = (Q,I)}$ is the gentle bound quiver~$\bar Q\blossom = (Q\blossom, I\blossom)$ obtained by adding arrows and relations at each vertex~$v \in Q_0$, so that~$v$ has precisely $2$ incoming and $2$ outgoing arrows and still fulfills the gentle quiver conditions. See Fig.~\ref{fig:exmQuiver}.

\begin{figure}[b]
	\centerline{\includegraphics[scale=.6]{exmQuiver}}
	\caption{A gentle bound quiver~$\bar Q$ (left), its blossoming quiver~$\bar Q\blossom$ (middle), and some walks in~$\bar Q\blossom$ (right). The dotted red and orange walks are non-kissing, but both are kissing the plain blue walk. See Fig.~\ref{fig:exmFlip} for examples of non-kissing facets.}
	\label{fig:exmQuiver}
\end{figure}
...
See Figs.~\ref{fig:exmQuiver}\,(right)~\&~\ref{fig:kissingFlip}\,(left).
...
In fact, the example of Fig.~\ref{fig:exmNKC} is also a special case of both families.

\begin{figure}[t]
	\centerline{\includegraphics[scale=.59]{exmNKC}}
	\caption{A reduced non-kissing complex (left) and its flip graph~(right).}
	\label{fig:exmNKC}
\end{figure}

\begin{figure}[b]
	\centerline{\includegraphics[scale=1.2]{kissingFlip}}
...
The following statement is inspired from~\cite[Thm.~3.2]{McConville} and illustrated in Fig.~\ref{fig:exmFlip}\,(left).

\begin{figure}[b]
	\centerline{\includegraphics[scale=.68]{exmFlip}}
...
See Figs.~\ref{fig:kissingFlip}\,(right)~and~\ref{fig:exmFlip}.~Then
\begin{compactenum}[(i)]
...
The flip operation of Prop.~\ref{prop:flip} and Figs.~\ref{fig:kissingFlip} \&~\ref{fig:exmFlip} exchanges two kissing walks~$\omega, \omega'$.
...
Congruence-uniform lattices will be properly defined in Sec.~\ref{subsec:joinRepresentations}.
To achieve Thm.~\ref{thm:lattice}, we use a technique developed by T.~McConville for grid quivers~\cite{McConville}: we identify the non-kissing lattice with a quotient of a lattice of biclosed sets of strings.
...
In~\cite[Thm.~5.5]{McConville}, T.~McConville provided simple sufficient conditions for~$\Bicl{\cS}$ to be a congruence uniform lattice.
In~\cite[Thm.~3.21]{PaluPilaudPlamondon}, we extended this criterion in the situation when the singletons of~$\cS$ are not biclosed so that we can apply it in our context of non-kissing complexes.
...
Fig.~\ref{fig:exmLatticeQuotient}\,(left) illustrates the poset~$\Bicl{\bar Q}$ for another gentle bound quiver, with the empty set in the bottom and the set of all strings of~$\bar Q$ on top. The criterion of~\cite[Thm.~3.21]{PaluPilaudPlamondon} yields the following~result.

\begin{theorem}
When~$\bar Q$ has finitely many strings, the inclusion poset of biclosed sets~$\Bicl{\bar Q}$ is a congruence-uniform lattice.

\begin{figure}[t]
	\centerline{\includegraphics[scale=.59]{exmLatticeQuotient}}
...
Following~\cite[Sect.~7]{McConville}, we associate to a biclosed set~$S \in \Bicl{\bar Q}$ the sets \\[.1cm]
\centerline{$
...
%These sets, illustrated in Fig.~\ref{fig:exmProjections}, satisfy the following properties.
%
%\begin{figure}[t]
%	\centerline{
%	\begin{overpic}[scale=.5]{exmProjections}
...
%	}
%	\vspace{.3cm}
%	\caption{A biclosed set of strings~$S$ (middle) with~$\projDown(S)$ (left) and~$\projUp(S)$~(right). For readibility, $\projUp(S)$ is only partially drawn: the remaining strings are obtained by adding independently the two dotted arrows to all strings containing their left endpoint.}
...
The congruence classes of~$\equiv$ appear as blue rectangles~in~Fig.~\ref{fig:exmLatticeQuotient}.

\subsection{Non-kissing lattice}

Coming back to our original problem, we now aim to show that the increasing flip graph on non-kissing facets is isomorphic to the Hasse diagram of the quotient of the lattice of biclosed set~$\Bicl{\bar Q}$ of Sec.~\ref{subsec:biclosedSets} by the lattice congruence of Sec.~\ref{subsec:congruence}.
The next two propositions provide explicit maps between biclosed sets of strings and non-kissing facets illustrated in Fig.~\ref{fig:exmSurjection}. These maps extend previous definitions of~\cite{McConville} for grid quivers.
...
\begin{proposition}
For any facet~$F \in \NKC$, the set of strings~$\zeta(F) \! \eqdef \! \displaystyle\closure{\Big( \!\!\bigcup\limits_{\omega \in F} \! \Sigma_\bottom(\omega) \! \Big)}
$~is~biclosed.
\end{proposition}
...
%
\begin{figure}[h]
	\centerline{$\raisebox{-1.5cm}{\begin{overpic}[scale=.65]{exmVectors}\put(60,-4){$F$}\end{overpic}}$ \quad \input{figures/gcmatrices}}
	\vspace{-.3cm}
...
Since the $\b{g}$-vectors of the walks~$\omega, \omega', \mu, \nu$ involved in the flip of Fig.~\ref{fig:kissingFlip} satisfy the linear dependence $\gvector{\omega} + \gvector{\omega'} = \gvector{\mu} + \gvector{\nu}$, we obtain the following~statement.
...
\begin{figure}[h]
	\centerline{\includegraphics[scale=.59]{exmFanAsso}}
	\caption{The $\b{g}$-vector fan~$\gvectorFan$ (left) and the gentle associahedra (right).}
	\label{fig:exmFans}
\end{figure}

This fan is illustrated in Fig.~\ref{fig:exmFans}\,(left).
...
An example of~$\Asso$ is shown in Fig.~\ref{fig:exmFans}.
...
%An example of~$\Asso$ is illustrated in Fig.~\ref{fig:exmFans}. 
...
This realization of the non-kissing complex has the following relevant property regarding the non-kissing lattice studied in Sec.~\ref{sec:nonkissinglattice}.
