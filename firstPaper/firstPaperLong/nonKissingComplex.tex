...
This involves in particular a precise description of the flip inspired from~\cite[Sect.~3]{McConville}.
...
Our main lattice-theoretic result is the following extension of a remarquable result of T.~McConville~\cite{McConville}.
...
\end{enumerate}
The algebra~$kQ/I$ is called a \defn{string algebra}.
\end{definition}
...
\begin{figure}[h]
	...
	\centerline{
		\begin{tabular}{c@{\qquad}|@{\qquad}c}
			...
			$I = (\alpha\beta, \gamma\delta, \alpha\delta)$ \\ \\ \hline \\
...
\\[.3cm]
Finally, both the Auslander-Reiten translation~$\tau$ and its inverse~$\tau^{-1}$ preserve the band modules: for a band~$\varpi$ of~$\bar Q$, we have~$\tau \big( M(\varpi) \big) = M(\varpi) = \tau^{-1} \big( M(\varpi) \big)$.
\end{theorem}
...
We say that the negative simple string $-v$ dances with the string $\sigma$ if~$v$ is a vertex of~$\sigma$.
On the other hand, no almost positive string dances with the negative simple string~$-v$.
\end{definition}
...
\begin{itemize}
\item $\Hom{A} (M, \tau P[1]) = 0$ for any representation or shifted projective~$M$,
\item $\Hom{A} (P[1], \tau M) = \Hom{A} (P,M)$ for any representation~$M$.
\end{itemize}
These choices are motivated by the isomorphism
\[
\Hom{A} (M, \tau N) \cong \Hom{K^b(\operatorname{proj} A)}(P_M, P_N[1]),
\]
...
\endxy
\]
	\caption{The string~$\rho$ dances with~$\rho'$, as in Definition~\ref{definition: dance and attract} (ii).}
...
Then \[\Hom{A} \big( M(\rho), \tau M(\rho') \big) \ne 0\] if and only if~$\rho$ attracts~$\rho'$ or dances with~$\rho'$.
\end{proposition}
...
Assume now that~$\rho$ dances with~$\rho'$, and let~$\xi$ be as in Definition~\ref{definition: dance and attract}.
...
\endxy
\]
    \caption{The string~$\rho''$, when~$\rho$ dances with~$\rho'$ and~$\rho'=\xi$ as in case (a).}
    \vspace{-.5cm}
...
\endxy
\]
    \caption{When~$\rho$ dances with~$\rho'$ and~$\rho'$ contains a substring~$\alpha\xi\beta^{-1}$ as in case~(b).}
    \vspace{-.5cm}
...
\endxy
\]
    \caption{When~$\rho$ dances with~$\rho'$ and~$\rho'=\xi\beta^{-1}\sigma$ as in case (c).}
    \vspace{-.5cm}
...
\item If~$\xi=\nu$. By Lemma~\ref{lemma: xi equals nu} below and the fact that~$\xi$ is on top of~$\rho$, we have that~$\rho$ dances with~$\rho'$. 
...
This shows that~$\rho$ dances with~$\rho'$ (we note that the swinging arms condition is satisfied).
...
The string~$\rho$ also contains~$\alpha^{-1}\xi\beta$ and therefore dances with~$\xi=\rho'$.
...
The support $\tau$-tilting complex of~$\bar Q$ is isomorphic to the clique complex of the graph whose vertices are the unoriented strings of~$\bar Q$ and whose edges link two strings which do not attract nor dance with each other.
\end{corollary}
...
\begin{example}
\fref{fig:exmFacet} shows examples of walks on the gentle quiver~$\bar Q\ex$ of \fref{fig:exmBlossomingQuiver}.
\end{example}
...
\begin{figure}[t]
	...
	\caption{An oriented path~$\bar Q\pa$ is simultaneously a triangulation and a grid bound quiver (top). A facet of the reduced non-kissing complex~$\RNKC[\bar Q\pa]$ can be seen as a triangulation or as a collection of non-kissing south-east paths (bottom).}
...
On the other hand, we can consider the maximal marked walk of~$F_\alpha$ for the order~$\prec_\alpha$.
...
\begin{proof}
We first prove that each walk of~$F$ contains at most one distinguished arrow in each direction.
...
\smallskip
We now prove that for any arrow~$\alpha$ on a walk~$\omega$ in a face~$F$ of~$\NKC$, there is a distinguished arrow on~$\omega \in F$ in the direction of~$\alpha$.
If~$\omega$ is not the distinguished walk of~$F$ at~$\alpha$, consider~${\mu \eqdef \min_{\prec_\alpha} \set{\nu \in F_\alpha}{\omega \prec_\alpha \nu}}$.
...
Let~$\lambda$ be the distinguished walk of~$F$ at~$\beta$.
...
Then~$\sigma$ attracts~$\sigma'$ or dances with~$\sigma'$ if and only if~$\omega(\sigma)$ kisses~$\omega(\sigma')$.
\end{lemma}
...
Assume that~$\sigma$ dances with~$\sigma'$, and let~$\xi$ be the substring on top of~$\sigma$ and at the bottom of~$\sigma'$, as in Definition~\ref{definition: dance and attract}\,(ii).
...
\endxy
\]
    \caption{The walks~$\omega(\sigma)$ and~$\omega(\sigma')$ when~$\sigma$ dances with~$\sigma'$: case (a).}
    \vspace{.1cm}
    \label{fig: dance case a}
\end{figure}
...
\endxy
\]
    \caption{The walks~$\omega(\sigma)$ and~$\omega(\sigma')$ when~$\sigma$ dances with~$\sigma'$: case (b).}
    \vspace{.1cm}
    \label{fig: dance case b}
\end{figure}
...
\endxy
\]
    \caption{The walks~$\omega(\sigma)$ and~$\omega(\sigma')$ when~$\sigma$ dances with~$\sigma'$: case (c).}
    \label{fig: dance case c}
\end{figure}
...
It is possible to show that $\sigma$ attracts or dances with $\sigma'$ by using the same method as in the proof of Proposition~\ref{proposition: attract or dance and tau compatibility}.
...
\begin{example}
The $\b{g}$-vector of the plain blue walk on \fref{fig:exmFacet}\,(left) is given by $(0,-1,0,0,1,0)$ (see \fref{fig:exmBlossomingQuiver} for the labeling order of the vertices of the quiver~$Q\ex$).
...
\begin{figure}[t]
	\capstart
	\centerline{$\raisebox{-1.5cm}{\begin{overpic}[scale=.6]{exmVectors}\put(60,4){$F\ex$}\end{overpic}}$ \qquad \input{gcmatrices}}
...
For a non-kissing facet~$F \in \RNKC$, the set of $\b{g}$-vectors~$\gvectors{F}$ and the set of $\b{c}$-vectors~$\cvectors{F}$ form dual bases. 
\end{proposition}
