...
\newcommand{\cS}{\mathbb{S}} % ground set
...
\hvFloat[floatPos=p, capWidth=h, capPos=r, capAngle=90, objectAngle=90, capVPos=c, objectPos=c]{table}
{
    ...
    Thm.~\ref{thm:nkc/sttiltc}
    \\ \hline
    \multiLinesBox{(left) mutation of support \\ $\tau$-tilting objects (Thm.~\ref{thm:mutationstautilts}~\cite{AdachiIyamaReiten})} &
...
\end{enumerate}
The algebra~$kQ/I$ associated to a gentle bound quiver~$\bar Q$ is called a \defn{gentle algebra}.
\end{definition}
...
\begin{example}
The string~$\rho = \gamma\beta^{-1}\alpha^{-1}\gamma\beta^{-1}\alpha^{-1}\gamma\delta$ of Example~\ref{exm:drawingString} in the quiver of Example~\ref{exm:aStringAlgebra} does not start on a deep, and we have
\[
\rho = \;\vcenter{\hbox{\includegraphics[scale=0.3]{exmString1}}}
\hspace{1.3cm}
...
Then the Auslander-Reiten translation transforms string modules to string modules: for a string~$\rho$ of~$\bar Q$, we have~${\tau \big( M(\rho) \big) = M(\rho')}$, where the string~$\rho'$ is obtained by following the table below.
\\[.3cm]
	\centerline{
	\begin{tabular}{|c|c|c|} 
      \hline 
...
\begin{figure}[t]
	...
	\caption{An oriented path~$\bar Q$ is simultaneously a triangulation and a grid bound quiver (top). A facet of the reduced non-kissing complex~$\RNKC$ can be seen as a triangulation or as a collection of non-kissing south-east paths (bottom).}
...
\begin{example}
\fref{fig:exmFacet}~(left) illustrates kissing and non-kissing walks.
%
\begin{figure}[t]
	\capstart
...
\begin{proof}
By Lemma~\ref{lem:distArrowExist}, the cardinality of any finite non-kissing face of~$\NKC$ is bounded by~$|Q_1\blossom|$.
...
\begin{corollary}
\label{coro:pure}
The non-kissing complex~$\NKC$ is pure and its dimension is ${|Q_0| + \Delta = 3|Q_0|-|Q_1|}$.
The reduced non-kissing complex~$\RNKC$ is pure and its dimension is $|Q_0|$.
\end{corollary}
...
We let~${\ascents(F) \subseteq \strings^\pm(\bar Q)}$ and $\descents(F) \subseteq \strings^\pm(\bar Q)$ denote the sets of ascents and descents of~$F$ respectively.
...
Note that when the quiver is an oriented path~$\bar Q$, a flip in a facet of the non-kissing complex~$\NKC$ corresponds to the classical flip of a diagonal in the corresponding triangulation (see \fref{fig:exmBijectionAssociahedron}).
...
Conversely, for a bending non-self-kissing walk~$\omega \in \NKWalks^\pm(\bar Q)$ not in the peak facet, the inclusion minimal string in the set~$\set{\sigma \in \Sigma_\bottom(\omega)}{\closure{\Sigma_\bottom(\omega)} = \closure{\Sigma_\bottom(\sigma)}}$ is denoted by~$\sigma_\bottom(\omega)$.
\end{definition}
...
As illustrated in \fref{fig:exmBijectionStringsWalks2}\,(bottom), the bijections of Lemma~\ref{lem:bijections-STTC-NKC} and Definition~\ref{def:bijectionWalksDistinguishableStrings} no longer coincide in general when not all strings are distinguishable.
\end{remark}
