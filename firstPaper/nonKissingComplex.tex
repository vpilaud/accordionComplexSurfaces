...
\begin{figure}[b]
	...
	\centerline{
		\begin{tabular}{c@{\quad}|@{\quad}c@{\quad}|@{\quad}c@{\quad}|@{\quad}c}
			...
			$Q = \vcenter{\hbox{\includegraphics[scale=.5]{exmQuiver2}}}$ &
			$Q = \vcenter{\hbox{\includegraphics[scale=.5]{exmQuiver3}}}$ &
			$Q = \vcenter{\hbox{\includegraphics[scale=.5]{exmQuiver4}}}$ \\[1.1cm]
...
On the other hand, \textcolor{red}{ the proposition below shows that} we can consider the maximal marked walk of~$F_\alpha$ for the order~$\prec_\alpha$.
...
\textcolor{red}{The facets of the non-kissing complex have finite cardinality.}
\end{proposition}

\begin{proof}\textcolor{red}{
Let~$F$ be a non-kissing facet.
...
Let~$\sigma$ be the longest common substring of~$\lambda$ and~$\nu$ containing~$\beta_1$ and~$\beta_2$ respectively.
If~$\sigma$ is on top of~$\nu$, then it is at the bottom of~$\lambda$, and~$\lambda$ kisses~$\nu$, which is absurd.
Otherwise, the subword~$\sigma$ of~$\mu$ containing~$\beta_2$ is at the bottom of~$\mu$ while the subword~$\sigma$ of~$\mu$ containing~$\beta_1$ is at the top of~$\mu$.
...
\begin{figure}[t]
	\capstart
	\centerline{$\raisebox{-1.5cm}{\begin{overpic}[scale=.7]{exmVectors}\put(60,4){$F\ex$}\end{overpic}}$ \qquad \input{gcmatrices}}
