...
A cellular dissection~$\dissection$ of~$(\surface, M)$ and a cellular dissection~$\dissection\dual$ of~$(\surface, M\dual)$ are \defn{dual cellular dissections} if there are pairs of mutually inverse bijections~$M\dual \leftrightarrow \faces(\dissection)$ and~$M \leftrightarrow \faces(\dissection\dual)$, denoted~$\dual$ in both directions, such that~$\dissection$ has an arc joining its vertices~$u,v \in M$ and separating its faces~$f,g \in  \faces(\dissection)$ if and only if~$\dissection\dual$ has an arc joining its vertices~$f\dual, g\dual \in M\dual$ and separating its faces~$u\dual, v\dual \in \faces(\dissection\dual)$.
\end{definition}
...
\begin{itemize}
\item if a face~$f$ has no arc on the boundary of~$\surface$, its dual vertex~$f\dual$ lies in the interior~of~$f$,
\item if a face~$f$ has an arc on the boundary of~$\surface$, then it has exactly one such arc and its dual vertex~$f\dual$ lies on this arc.
\end{itemize}
...
For a cellular dissection~$\dissection$ of a marked surface~$(\surface, M)$, the following assertions are equivalent:
\begin{enumerate}[(i)]
\item there exists a set~$M\dual$ of points of~$\surface$ and a cellular dissection~$\dissection\dual$ of~$(\surface, M\dual)$ such that~$\dissection$ and~$\dissection\dual$ are dual cellular dissections,
\item each face of~$\dissection$ has at most one boundary arc.
\end{enumerate}
Moreover, the set~$M\dual$ and the cellular dissection~$\dissection\dual$ are both uniquely determined.
\end{proposition}

\begin{proof}
We mimick the proof of~\cite[Proposition 1.12]{OpperPlamondonSchroll}.
...
\yann{Il faut probablement ajouter des conditions aux bords}
\end{definition}
...
\begin{definition}
\label{def:zigzag}
A \defn{$\dissection\dual$-zigzag} is an arc~$\alpha$ of~$(\surface, B)$ such that, for each pair of faces~$(f,g) \in \faces(\dissection\dual)$ that~$\alpha$ enters consecutively by crossing~$\beta\in\dissection\dual$,~$\alpha$ cuts the union of the two cells $f$ and $g$ glued along~$\beta$ into two components, one containing the marked point $f\dual$ and the other containing $g\dual$.
\end{definition}
...
For two dual cellular dissections~$\dissection$ and~$\dissection\dual$, the $\dissection$-accordions are precisely the $\dissection\dual$-zigzags and the $\dissection$-accordion complex coincides with the $\dissection\dual$-zigzag complex.
\end{proposition}
...
One conclude that~$\alpha$ is not a~$\dissection\dual$-zigzag similarly as in the first case, by considering the component of~$m\dual\setminus\alpha$ that do not contain~$m$.
...
We call \defn{non-crossing complex} of the pair of dual cellular dissections~$(\dissection, \dissection\dual)$ the simplicial complex~$\NCC = \AC = \ZC$.
\end{definition}
