...
The surface~$\surface(\bar Q)$ of a locally gentle bound quiver~$\bar Q = (Q,I)$ is the orientable surface obtained from the blossoming quiver~$\bar Q\blossom$ as follows:
\begin{itemize}
\item for each arrow~$\alpha \in Q_1\blossom$, consider a lozange~$L(\alpha)$ with four (oriented) sides~$\Enrs{\alpha} = [s(\alpha), v]$, $\Enrt{\alpha} = [v, t(\alpha)]$ (colored green) and~$\Ers{\alpha} = [s(\alpha), v\dual]$ and~$\Ert{\alpha} = [v\dual, t(\alpha)]$ (colored red) as follows:
\vincent{Figure}
\item for any~$\alpha, \beta \in Q_1\blossom$ with~$t(\alpha) = s(\alpha)$, proceed to the following identifications:
    ...
    \end{itemize}
\end{itemize}
The construction also automatically defines two dual cellular dissections~$\dissection_{\bar Q}$ and~${\dissection_{\bar Q}}\dual$ of the surface~$\surface_{\bar Q}$.
\end{definition}
