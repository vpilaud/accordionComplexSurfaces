...
We are also grateful to Salvatore Stella for discussions on accordion complexes of dissections of surfaces, and to William Crawley-Boevey and Henning Krause for discussions on the representation theory of infinite dimensional algebras. 
...
A \defn{locally gentle bound quiver}~$\bar Q \eqdef (Q,I)$ is a (finite) bound quiver such that
\begin{itemize}
...
A \defn{finite string} for~$\bar Q$ is a word of the form
\(
...
We note that there is only one string of length 
...
\begin{itemize}
 ...
 \item $c^\infty$ for the word:~$c c c\cdots$;
 \item $^{-\infty} c$ for the infinite word:~$\cdots c^{-1} c^{-1} c^{-1}$;
 \item $c^{-\infty}$ for the word:~$c^{-1} c^{-1} c^{-1}\cdots$ and
 \item $^\infty c^\infty$ for the word:~$^\infty c c^\infty$.
\end{itemize}
\end{notation}

We note that~$(c^\infty)^{-1} =\,^{-\infty}c$ and similarly~$(^\infty c)^{-1} = c^{-\infty}$.

\begin{definition}
An \defn{eventually cyclic string} for~$\bar Q$ is a word~$\rho$ of the form~$^{\pm\infty}c_1\sigma c_2^{\pm\infty}$, where $c_1,c_2$ are oriented cycles in~$\bar Q$ (possibly of length zero), $c_1, c_1^2, c_2, c_2^2\notin I$, and~$\sigma$ is a finite string.
...
\begin{remark}
 The terminology in the previous definition is borrowed from~\ref{Crawley-Boevey}.
 Note however that our choice of convention differs from \emph{loc. cit.}, resulting in exchanging ``inverse'' and ``direct''.
\end{remark}
...
A \defn{substring} of a walk~$\omega =\cdots \alpha_{-r}^{\varepsilon_{-r}}\cdots\alpha_0^{\varepsilon_0} \alpha_1^{\varepsilon_1} \cdots \alpha_\ell^{\varepsilon_\ell}\cdots \in \walks(\bar Q)$ is a string~$\sigma = \alpha_i^{\varepsilon_i} \dots \alpha_j^{\varepsilon_j}$ of~$\strings(\bar Q)$ for some~$i \le j \in \mathbb{Z}\cup\{\pm\infty\}$, where moreover
\begin{itemize}
...
The substring~$\sigma = \alpha_i^{\varepsilon_i} \dots \alpha_j^{\varepsilon_j}$ is \defn{at the bottom} (resp.~\defn{on top}) of the walk~$\omega = \alpha_1^{\varepsilon_1} \dots \alpha_\ell^{\varepsilon_\ell}$ if~$i=-\infty$ or~$\varepsilon_{i-1} = 1$ and~$j=+\infty$ or~$\varepsilon_{j+1} = -1$ (resp.~if~$i=-\infty$ or~$\varepsilon_{i-1} = -1$ and~$i=-\infty$ or~$\varepsilon_{j+1} = 1$), that is, the (at most) two arrows of~$\omega$ incident to the endpoints of~$\sigma$ point towards~$\sigma$ (resp.~outwards from~$\sigma$).
...
The corresponding perfect matchings (see Remark~\ref{rem:propertiesSurface}) look like~\raisebox{-.1cm}{\includegraphics[scale=.3]{reversedPathsPM}}.
...
The corresponding perfect matchings (see Remark~\ref{rem:propertiesSurface}) look like~\raisebox{-.1cm}{\includegraphics[scale=.3]{doublePathsPModd}} when~$|Q_0|$ is odd and~\raisebox{-.1cm}{\includegraphics[scale=.3]{doublePathsPMeven}} when~$|Q_0|$ is even.
...
Let~$\gamma = \curveof(\omega)$.  The fact that~$\omega$ contains~$c$ translates to the fact that~$\gamma$ makes a full circle around the correponding puncture.
\pierreguy{Pic No 8 here.}

\includegraphics{infiniteCurve2.pdf}
...
\vincent{Should we also give here a geometric argument?}
\end{proof}
...
Construct a curve~$\gamma$ as follows:~$\gamma$ first unwinds from the puncture, then~$\gamma$ leaves the local picture around the puncture by turning aroune another marked point (marked~$X$ on the picture below).

\includegraphics{infiniteCurve1.pdf}

\pierreguy{Pic No 7 here.}
...
\vincent{Should we also give here a geometric argument? Pierre-Guy, could you add that?}
\pierreguy{Done; I will need a picture, please, Vincent.}
...
\section{$\b{g}$-, $\b{c}$- and $\b{d}$-vectors}

\vincent{To do on the walks, on the accordions and on the slaloms.}

In this section, we recall some results from Section 4.2 of~\cite{PaluPilaudPlamondon} that immediately generalise to the case of locally gentle algebras.
...
The duality between~$\b{g}$-vectors and~$\b{c}$-vectors then follows from the duality between the two dissections~$\dissection_{\bar Q}$ and~$\dissection\dual_{\bar Q}$.
\yann{check the lattest assertion}
...
Observe that~$\gvector{\omega} = 0$ for a straight walk~$\omega$, and that, since for any walk~$\omega$, the strings $\omega$ and $\omega^{-1}$ are eventually direct or inverse, the coordinates of~$\gvector{\omega}$ are finite.
\end{definition}
...
\begin{definition}\label{def: c-vectors for walks}
...
Recall that there is a fixed bijection between the vertices of~$Q$, the arc in the dissection~$\dissection_{\bar Q}$ and the arcs in the dual dissection~$\dissection^\dual_{\bar Q}$.

\begin{definition}\label{def: c-vectors for accordions}
Let~$\alpha$ be a~$\dissection_{\bar Q}$-accordion in a non-crossing facet~$F$ of~$\RNCC$.
The distinguished curve of~$\alpha$ in~$F$ is the part of~$\alpha$ which lies strictly between the two angles of~$\dissection_{\bar Q}$ where~$\alpha$ is maximal (see Remark~\ref{rema:countercurrentOrderOnSurface}).
The~$\b{c}$-vector of~$\alpha$ in~$F$ is the vector in~$\R^{Q_0}$ counting the number of crossings between the dinstinguished curve of~$\alpha$ in~$F$ and the arcs of the dissection~$\dissection_{\bar Q}$.
\end{definition}

\begin{definition}\label{def: g-vectors for slaloms}
Let~$\alpha$ be a~$\dissection\dual_{\bar Q}$-slalom.
The~$\b{g}$-vector of~$\alpha$ is the vector in~$\R^{Q_0}$ counting the number of times that~$\alpha$ positively crosses each arc of~$\dissection\dual_{\bar Q}$ minus the number of times it crosses each arc negatively.
Here, a crossing is positive when~$\alpha$ leaves a face~$f^\dual$ to enter a face~$g^\dual$ with the vertex~$f$ being to its right (and thus with~$g$ being to its left).
Otherwise, the crossing is negative.
\end{definition}

\yann{Ajouter une phrase pour mettre en avant cette proposition}

\begin{proposition}\label{prop: geometric interpretation of g- and c-vectors}
 Let~$\omega$ be a non-kissing walk in~$\bar Q$, and let~$\alpha$ be the corresponding curve on~$\surface(\bar Q)$, as in Definition~\ref{def:curveOfAWalk}.
 Recall from Lemma~\ref{lemm:curveOfAWalkIsAccordion} and Proposition~\ref{prop:accordionsSlaloms} that~$\alpha$ is both a~$\dissection_{\bar Q}$-accordion and a~$\dissection^\dual_{\bar Q}$-slalom.
 Let~$F$ be a non-kissing facet to which~$\omega$ belong.
 We also write~$F$ for the corresponding non-crossing facet in the slalom complex.
 Then:
 \begin{enumerate}
  \item The~$\b{g}$-vectors of the walk~$\omega$ and the accordion~$\alpha$ coincide.
  \item The~$\b{c}$-vectors of the walk~$\omega$ in~$F$ and the slalom~$\alpha$ in~$F$ concide.
 \end{enumerate}
\end{proposition}

\begin{proof}
Definitions~\ref{def: c-vectors for accordions} and~\ref{def: g-vectors for slaloms} are immediate translations in terms of curves on the surface~$\surface(\bar Q)$ of Definitions~\ref{def: c-vectors for walks} and~\ref{def: g-vectors for walks} respectively.
\end{proof}

\begin{remark}
Proposition~\ref{prop: geometric interpretation of g- and c-vectors} gives a new proof of Proposition~\ref{prop:gvectorscvectorsDualBases}: \yann{To Do}
\end{remark}

\begin{definition}
The \defn{$\b{g}$-vector fan} of~$\bar Q$ is the collection of cones
\[
...
\yann{Th 4.17 and Th 4.27 of [PPP1] seem to apply as such here. Vincent?}
Theorem 4.17 and Theorem 4.27 of~\cite{PaluPilaudPlamondon} hold for locally gentle algebras, with the same proofs:

\begin{theorem}
Let~$\bar Q$ be a locally gentle bound quiver. Then: 
\begin{enumerate}
 \item the~$\b{g}$-vector fan of~$\bar Q$ is a complete simplicial fan;
 \item If moreover~$\NKC$ is finite, then $\gvectorFan$ is the normal fan of the~$\bar Q$-associahedron.
\end{enumerate}
\end{theorem}
