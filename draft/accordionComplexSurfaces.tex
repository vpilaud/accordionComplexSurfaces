...
\usepackage{enumerate, amsmath, amsfonts, amssymb, amsthm, mathrsfs, wasysym, graphics, graphicx, xcolor, url, hyperref, hypcap, xargs, multicol, pdflscape, multirow, hvfloat, array, ae, aecompl, pifont, mathtools, a4wide, float, blkarray, chemmacros}
...
Namely, any pair of dual cellular dissections~$(\dissection, \dissection\dual)$ defines a locally gentle bound quiver~$\bar Q_{\dissection}$ such that the non-crossing complex~$\NCC$ is isomorphic to the non-kissing complex~$\NKC[\bar Q_{\dissection}]$.
Conversely, any locally gentle bound quiver~$\bar Q$ gives rise to a surface~$\surface_{\bar Q}$ equipped with a pair of dual cellular dissections~$\dissection_{\bar Q}$ and~${\dissection\dual}\!\!_{\bar Q}$ such that the non-kissing complex~$\NKC$ is isomorphic to the non-crossing complex~$\NCC[\dissection_{\bar Q}, {\dissection\dual_{\bar Q}}]$.
...
\end{lemma}
\begin{proof}
Each arc~$a$ has two endpoints, and at each endpoint there is at most one arc coming immediately after~$a$ in the clockwise order, and at most one coming immediately before~$a$.
...
The advantage of Definition~\ref{def:surfaceQuiver} is that it automatically endows~$\surface_{\bar Q}$ with two disjoint sets~$V_{\bar Q}$ and~${V\dual_{\bar Q}}$ of marked points and two dual cellular dissections~$\dissection_{\bar Q}$ and~${\dissection\dual_{\bar Q}}$ defined as follows.
...
\end{itemize}
The set~${V\dual_{\bar Q}}$ and the ${V\dual_{\bar Q}}$-dissection~${\dissection\dual_{\bar Q}}$ are defined dually.
\end{definition}

\begin{proposition}
\label{prop:dissectionsAreCellular}
Let~$\bar Q$ be a locally gentle bound quiver.
Then the dissections~$\dissection_{\bar Q}$ and~$\dissection\dual_{\bar Q}$ are cellular and dual to each other.
\end{proposition}
\begin{proof}
...
\item \label{item:arcs}
      The dissection~$\dissection_{\bar Q}$ has one edge for each vertex~$v \in \bar Q_0$, obtained by concatenation of the sides~$\Enrt{\alpha} = \Enrs{\beta}$ and~$\Enrt{\alpha'} = \Enrs{\beta'}$ where~$v = t(\alpha) = s(\beta) = t(\alpha') = s(\beta')$, $\alpha\beta \notin I$ and~$\alpha'\beta' \notin I$. We denote by~$\edgeof(v)$ the edge of~$\dissection_{\bar Q}$ corresponding to~$v$.
\item The dissection~$\dissection_{\bar Q}$ has one $\ell$-cell for each straight walk of length~$\ell$ in~$\bar Q\koszul$.
\item A similar remark holds dually for~${V\dual_{\bar Q}}$ and~${\dissection\dual_{\bar Q}}$.
...
For any gentle bound quiver~$\bar Q$ with Koszul dual~$\bar Q\koszul$, the surfaces~$\surface_{\bar Q}$ and~$\surface_{\bar Q\koszul}$ coincide, but~$\dissection_{\bar Q\koszul} = {\dissection\dual_{\bar Q}}$ and~${\dissection\dual_{\bar Q\koszul}} = \dissection_{\bar Q}$.
\end{proposition}
...
A notable difference is that in \cite{OpperPlamondonSchroll}, only~$\dissection_{\bar Q}$ is given, and~$\dissection\dual_{\bar Q}$ is deduced (it is called the dual lamination).
...
The first few corresponding surfaces are represented in \fref{fig:cyclesSurfacesBis} (see Figures~\ref{fig:surfaces} and~\ref{fig:reversedPathsSurfaces} for the cycles with $1$ or~$2$ vertices, recalling that~$\bar Q$ and~$\bar Q\koszul$ yield the same surface).
...
Two successive intersection points in this sequence define an angle between two arcs of~$\dissection_{\bar Q}$, which corresponds to an arrow in~$\bar Q$.
...
\begin{itemize}
\item for any locally gentle bound quiver~$\bar Q$, the non-kissing complex~$\NKC$ is isomorphic to the non-crossing complex~$\NCC[\dissection_{\bar Q}, {\dissection\dual_{\bar Q}}]$,% therefore to the accordion complex~$\AC[\dissection_{\bar Q}]$ and to the slalom complex~$\SC[{\dissection\dual_{\bar Q}}]$.
...
\begin{remark}
Note that our construction of the surface~$\surface_{\bar Q}$ implies that the vertices of~$\bar Q$ are naturally identified with the intersection points of the dual edges~$a \in \dissection_{\bar Q}$ and~$a\dual \in {\dissection\dual_{\bar Q}}$.
...
We have seen in Definition~\ref{def:straightBending} that each vertex~$v$ of~$Q$ gives rise to a \defn{peak walk}~$v_\peak$ (resp.~\defn{deep walk}~$v_\deep$) with a single peak (resp.~deep) at~$v$ and no other corner.
The set of all such walks forms the \defn{peak facet}~$\set{v_\peak}{v \in Q_0}$ (resp.~the \defn{deep facet}~$\set{v_\deep}{v \in Q_0}$) of the non-kissing complex~$\RNKC$.
...
We invite the reader to check that~$\curveof(v_\peak) = \vprevious{\edgeof(v)}$ (resp.~$\curveof(v_\deep) = \vnext{\edgeof(v)}$), where~$\edgeof(v)$ denotes the edge of~$\dissection$ corresponding to~$v \in Q_0$ (see Remark~\ref{rem:propertiesSurface}).
...
\begin{definition}\label{def:orderForWalks}
...
\end{lemma}
\begin{proof}
 We provide two proofs of the lemma.  One is combinatorial and applies to~$\NKC$, and the other is geometric and applies to~$\NCC[\dissection_{\bar Q}, {\dissection\dual_{\bar Q}}]$, using the isomorphism of Theorem \ref{thm:complexesCoincide} and Remark \ref{rem:countercurrentOrderOnSurface}.
 ...
 View~$F$ as a face of the non-crossing complex~$\NCC[\dissection_{\bar Q}, {\dissection\dual_{\bar Q}}]$.
...
Any face of~$\NKC$ and~$\NCC[\dissection_{\bar Q}, {\dissection\dual_{\bar Q}}]$ is finite, thus contained in a finite maximal face.
\end{proposition}

Even though Theorem \ref{thm:complexesCoincide} ensures that~$\NKC$ and~$\NCC[\dissection_{\bar Q}, {\dissection\dual_{\bar Q}}]$ coincide, we provide two proofs.
One is combinatorial and applies to~$\NKC$, and the other is geometric and applies to~$\NCC[\dissection_{\bar Q}, {\dissection\dual_{\bar Q}}]$.
...
\begin{proof}
 ...
 \para{Geometric proof}
 Let~$F$ be a face of the non-crossing complex~$\NCC[\dissection_{\bar Q}, {\dissection\dual_{\bar Q}}]$.
...
\begin{proof}[Proof of Lemma~\ref{prop:facetsAreFinite}]
...
\begin{proposition}
\label{prop:purity}
The reduced non-kissing complex~$\RNKC$ and the reduced non-crossing complex~$\RNCC[\dissection_{\bar Q}, {\dissection\dual_{\bar Q}}]$ are pure of dimension~$|Q_0|$.
The non-kissing complex~$\NKC$ and the non-crossing complex~$\NCC[\dissection_{\bar Q}, {\dissection\dual_{\bar Q}}]$ are pure of dimension~$3|Q_0| - |Q_1| + s_\infty$ where~$s_\infty$ is the number of primitive oriented cycles in~$\bar Q$ or equivalently the number of punctures in~$V_Q$ in~$\surface_{\bar Q}$.
\end{proposition}
...
Note also that, by Theorem \ref{thm:complexesCoincide} above,~$\NKC$ and~$\NCC[\dissection_{\bar Q}, {\dissection\dual_{\bar Q}}]$ coincide, so it suffices to prove the Proposition for one of them.
...
Consider the marked walk~$\omega_\star \eqdef \max_{\prec_\alpha} F_\alpha$ (which exists by Lemmas \ref{lemm:facesHaveMaximalElement} and~\ref{prop:facetsAreFinite}),
...
Let~$b$ denote the number of bending walks, $s$ the number of finite straight walks, and $s_\infty$ the number of infinite straight walks in a non-kissing facet~$F$ of~$\NKC$.
...
\[
2 * b + 1 * s + 0 * s_\infty= |Q_1\blossom| = 4|Q_0|-|Q_1|
\]
from which we derive that~$b = |Q_0|$ and~$b+s+c = 3|Q_0|-|Q_1|+s_\infty$.
\end{proof}
...
\begin{definition}
Let~$V \eqdef \{v_1, \dots, v_m\}$ be a multiset of vertices of~$Q_0$. We denote by~$\multiplicityVector_V \in \R^{Q_0}$ the \defn{multiplicity vector} of~$V$, defined~by:
\[
...
The \defn{$\b{g}$-vector} of the walk~$\omega$ is the vector~${\gvector{\omega} \in \R^{Q_0}}$ defined as follows:
\[
...
Observe that, if~$\omega$ is a straight walk, then~$\gvector{\omega} = 0$, and that, since any infinite walk~$\omega$ is eventually cyclic, the coordinates of~$\gvector{\omega}$ are finite.
\end{definition}
...
\begin{cases}
\phantom{-}1 & \text{if~$\distinguishedString{\omega}{F}$ is a top substring of~$\omega$ (\ie if the distinguished arrows point outside),} \\
-1 & \text{if~$\distinguishedString{\omega}{F}$ is a bottom substring of~$\omega$ (\ie if the distinguished arrows point inside).}
\end{cases}
\]
...
\displaystyle \sum_{v \in Q_0} \kn(\omega, v_\deep) \, \b{e}_v & \text{otherwise.}
\end{cases}
\]
...
According to Remark~\ref{rem:propertiesSurface}\,(\ref{item:arcs}), each vertex~$p$ of~$Q$ corresponds to an edge~$\edgeof(p)$ in the dissection~$\dissection_{\bar Q}$ and  to an edge~$\dualedgeof(p) = \edgeof(p)\dual$ in the dual dissection~${\dissection\dual_{\bar Q}}$.
...
For a vertex~$v \in Q_0$, the $v$-th entry of the $\b{c}$-vector~$\cvector{\omega}{F}$ is given by:
\begin{enumerate}[(i)]
...
For a walk~$\omega$ of~$\bar Q$ and a vertex~$v \in Q_0$, the $v$-th entry of the $\b{d}$-vector~$\dvector{\omega}$ is the number of crossings of the curve~$\gamma(\omega)$ with the edge of~$X_\deep$ corresponding to~$v$.
\end{proposition}
