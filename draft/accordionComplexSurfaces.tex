...
It was shown in \cite{PaluPilaudPlamondon} that these complexes are special cases of a more general simplicial complex, defined for any gentle algebra (see also \cite{}).
...
Consider the family of ``reversed paths quivers'' indicated in \fref{fig:reversedPathsQuivers}.

\begin{figure}[h]
	\capstart
	\centerline{\includegraphics[scale=.45]{reversedPathsQuivers}}
	\caption{The reversed paths quivers with~$1, 2, 3, 4$ vertices.}
...
\begin{figure}[h]
	\capstart
	\centerline{\includegraphics[scale=.4]{reversedPathsSurfaces}}
	\caption{The surface~$\surface_{\bar Q}$ for the reversed paths with~$1, 2, 3, 4$ vertices.}
...
Consider the family of ``double paths quivers'' indicated in \fref{fig:doublePathsQuivers}.
...
\begin{figure}[h]
	...
	\caption{The order~$\prec_\alpha$ on marked walks is the order in which the corresponding curves cross the angle corresponding to~$\alpha$.}
...
\begin{proof}
 ...
 The first two dots in this proposition are proven in Lemma~\ref{lem:ExistenceOfDistinguishedArrows}.
\end{proof}
