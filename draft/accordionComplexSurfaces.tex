...
\begin{definition}
\label{def:walk}
A \defn{walk} of a locally gentle quiver~$\bar Q$ is a maximal string of its blossoming quiver~$\bar Q \blossom$, meaning that at each end it either reaches a blossom vertex of~$\bar Q \blossom$ or enters an infinite cycle.
...
It is not hard to see that~$\bar Q_{\dissection} = (Q_{\dissection}, I_{\dissection})$ is a locally gentle bound quiver,
...
\begin{figure}[t]
	\capstart
	\centerline{\includegraphics[scale=.4]{koszulQuivers}}
...
\begin{tikzpicture}[
	...
	roundNode/.style={circle, fill=black, inner sep=2pt, outer sep=2pt}
	]
	\node[roundNode, label=left:{$s(\alpha)$}] (s) at (-1,0) {};
	\node[roundNode, label=right:{$t(\alpha)$}] (t) at (1,0) {};
	\node[roundNode, fill=green, label=above:{$\green v(\alpha)$}] (v) at (0,1) {};
	\node[roundNode, fill=red, label=below:{$\red v\dual(\alpha)$}] (vd) at (0,-1) {};
	...
	\draw[color=green, postaction={decorate}] (v) -> (s) node[midway, above left] {$\green \Enrs{\alpha}$};
	\draw[color=green, postaction={decorate}] (v) -> (t) node[midway, above right] {$\green \Enrt{\alpha}$};
	\draw[color=red, postaction={decorate}] (vd) -> (s) node[midway, below left] {$\red \Ers{\alpha}$};
	\draw[color=red, postaction={decorate}] (vd) -> (t) node[midway, below right] {$\red \Ert{\alpha}$};
\end{tikzpicture}
}
...
\begin{figure}[t]
	...
	\caption{The surface~$\surface_{\bar Q}$ for the first four quivers~$\bar Q$ of \fref{fig:quivers}. The boundaries of the surfaces are shaded (the second surface has no boundary, it is just a sphere).}
	\label{fig:examplesSurfaces}
\end{figure}
\begin{figure}[t]
	\capstart
	\centerline{\includegraphics[scale=.7]{examplesSurfaces2}}
	\caption{The surface~$\surface_{\bar Q}$ for the last two quivers~$\bar Q$ of \fref{fig:quivers}. The boundaries of the surfaces are shaded. The first surface is a cylinder, the second one is a torus with a hole.}
	\label{fig:examplesSurfaces2}
\end{figure}
%
...
\item $\dissection_{\bar Q}$ has one $\ell$-cell for each straight walk of length~$\ell$ in~$\bar Q\koszul$.
\end{itemize}
...
For any face~$F$ of~$\NKC$ and any arrow~$\alpha\in Q_1\blossom$, the set~$F_\alpha$ admits a maximal element for the countercurrent order~$\prec_\alpha$.
\end{lemma}
\begin{proof}
 ...
 The arrow~$\alpha$ then corresponds to an angle in~$\dissection$.  
 
 The result is trivial if the set~$F_\alpha$ is finite; assume, therefore, that it is infinite.
...
\begin{proposition}
\label{prop:purity}
 The the non-kissing complex~$\NKC$ and the non-crossing complex~$\NCC[\dissection_{\bar Q}, {\dissection_{\bar Q}}\dual]$ are pure of dimension~$|Q_0| + \Delta$, where~$\Delta$ is the number of blossoming vertices of~$Q\blossom$ divided by two.
\end{proposition}
