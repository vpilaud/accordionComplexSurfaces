...
\begin{definition}
\label{def:topBottom}
The substring~$\sigma = \alpha_i^{\varepsilon_i} \dots \alpha_j^{\varepsilon_j}$ is \defn{at the bottom} (resp.~\defn{on top}) of the walk~$\omega = \cdots \alpha_1^{\varepsilon_1} \dots \alpha_\ell^{\varepsilon_\ell} \cdots $ if~$i=-\infty$ or~$\varepsilon_{i-1} = 1$ and~$j=+\infty$ or~$\varepsilon_{j+1} = -1$ (resp.~if~$i=-\infty$ or~$\varepsilon_{i-1} = -1$ and~$j=\infty$ or~$\varepsilon_{j+1} = 1$), that is, the (at most) two arrows of~$\omega$ incident to the endpoints of~$\sigma$ point towards~$\sigma$ (resp.~outwards from~$\sigma$).
