...
\newcommand{\arcof}{\operatorname{\alpha}} % arc of a vertex
\newcommand{\dualarcof}{\operatorname{\alpha}\dual} % dual arc of a vertex
...
The \defn{reduced non-kissing complex}~$\RNKC$ is the simplicial complex whose faces are the collections of pairwise non-crossing bended walks of~$\bar Q$.
\end{definition}
...
According to Remark~\ref{rem:propertiesSurface}\,(\ref{item:arcs}), each vertex~$p$ of~$Q$ corresponds to an arc~$\arcof(p)$ in the dissection~$\dissection_{\bar Q}$ and  to an arc~$\dualarcof(p) = \arcof(p)\dual$ in the dual dissection~${\dissection_{\bar Q}}\dual$.
...
\begin{enumerate}[(i)]
\item The number of times that the $\dissection$-accordion~$\curveof(\omega)$ positively crosses the arc~$\arcof(p)$ of~$\dissection$ minus the number of times it negatively crosses the arc~$\arcof(p)$ of~$\dissection$. Here, the crossing is positive (resp.~negative) if the two angles of~$\dissection$ crossed by~$\curveof(\alpha)$ just before and after~$\arcof(p)$ form a~$\SSS$ (resp.~as a~$\ZZZ$).
\item The number of times that the $\dissection\dual$-slalom~$\curveof(\omega)$ positively crosses the arc~$\dualarcof(p)$ of~$\dissection\dual$ minus the number of times it negatively crosses the arc~$\dualarcof(p)$ of~$\dissection\dual$. Here, the crossing is positive when~$\curveof(\omega)$ leaves a face~$u\dual$ to enter a face~$v\dual$ with the vertex~$u$ being to its right (and thus with~$v$ being to its left), and the crossing is negative otherwise.
\end{enumerate}
\end{proposition}
...
For a walk~$\omega \in \NKWalks^\pm(\bar Q)$ and a vertex~$p \in Q_0$, the $p$th entry of the $\b{d}$-vector~$\dvector{\omega}$ is the number of crossings of the curve~$\gamma(\omega)$ with the arc of~$X_\deep$ corresponding to~$p$.
\end{proposition}
