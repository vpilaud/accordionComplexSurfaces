...
We consider a \defn{bound quiver}~$\bar Q = (Q,I)$, formed by a quiver~$Q$ and an admissible ideal~$I$ of the path algebra~$kQ$.
...
\begin{definition}
\label{def:walk}
A \defn{walk} of a gentle quiver~$\bar Q$ is a maximal string of its blossoming quiver~$\bar Q \blossom$, meaning that at each end it either reaches a blossom vertex of~$\bar Q \blossom$ or enters an infinite cycle.
...
Case 4: If $\alpha$ stays inside a blue cell, then it rolls around a $\dissection\dual$ puncture. There is a $\dissection\dual$-arc $\beta$ ending in that puncture. When $\alpha$ crosses $\beta$, the puncture stays on the same side of $\alpha$, hence $\alpha$ is not a $\dissection\dual$-slalom.
\end{proof}
...
Conversely, any gentle bound quiver~$\bar Q$ gives rise to a surface~$\surface_{\bar Q}$ equipped with a pair of dual cellular dissections~$\dissection_{\bar Q}$ and~${\dissection_{\bar Q}}\dual$ such that the non-kissing complex~$\NKC$ is isomorphic to the non-crossing complex~$\NCC[\dissection_{\bar Q}, {\dissection_{\bar Q}}\dual]$.
...
The \defn{Koszul dual} of a gentle bound quiver~$\bar Q = (Q,I)$ is the bound quiver~$\bar Q\koszul = (Q\koszul, I\koszul)$ defined as follows:
\begin{itemize}
 \item the quiver~$Q\koszul$ is equal to~$Q$;
 \item the ideal~$I\koszul$ is generated by the paths of length two in~$Q$ that do not appear in~$I$.
\end{itemize}
\end{definition}
\pierreguy{Justify this definition.}
Note that~$\bar Q\koszul$ is gentle, and that~$(\bar Q\koszul)\koszul = \bar Q$.
...
The surface~$\surface(\bar Q)$ of a gentle bound quiver~$\bar Q = (Q,I)$ is obtained by glueing lozanges around arrows of~$Q$ following the rule given by the relations~$I$. The construction also automatically defines two dual cellular dissections~$\dissection_{\bar Q}$ and~${\dissection_{\bar Q}}\dual$ of the surface~$\surface_{\bar Q}$.
\end{definition}
...
Non-kissing walks = non-crossing $\dissection$-accordion curves = non-crossing $\dissection\dual$-slalom curves.
\end{lemma}

\begin{theorem}
\label{thm:complexesCoincide}
\begin{itemize}
\item For any gentle bound quiver~$\bar Q$, the non-kissing complex~$\NKC$ is isomorphic to the non-crossing complex~$\NCC[\dissection_{\bar Q}, {\dissection_{\bar Q}}\dual]$, therefore to the accordion complex~$\AC[\dissection_{\bar Q}]$ and to the slalom complex~$\SC[{\dissection_{\bar Q}}\dual]$.
...
In this section, we show that the non-crossing and non-kissing complexes are \defn{pseudomanifold}, \ie that there are
\begin{enumerate}[(i)]
\item \defn{pure}: all maximal $\dissection$-accordion simplices have the same number of $B$-curves as~$\dissection$, and
\item \defn{thin}: any codimension~$1$ simplex of~$\AC$ is contained in exactly two maximal $\dissection$-accordion simplices.
\end{enumerate}

\vincent{Peut-on utiliser une projection d'une alg\`ebre ammass\'ee ? Apparement non... On peut avoir le Koszul dual d'une alg\`ebre ammass\'ee.}
