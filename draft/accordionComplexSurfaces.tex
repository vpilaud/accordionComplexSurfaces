...
They have been defined in studied in \cite{Baryshnikov, Chapoton-quadrangulations, GarverMcConville, MannevillePilaud-accordion} in the case where the surface is an unpunctured disk.
...
The terminology in the following definitions is borrowed from~\cite{ButlerRingel, Crawley-Boevey}.
...
\begin{definition}
An \defn{eventually cyclic string} for~$\bar Q$ is a word~$\rho$ of the form~${}^\infty(c_1^{\varepsilon_1})\sigma (c_2^{\varepsilon_2})^\infty$, where $c_1, c_2$ are oriented cycles in~$\bar Q$ (possibly of length zero) and $\varepsilon_1, \varepsilon_2$ are signs in~$\{\pm 1\}$ such that $c_1^{2\varepsilon_1} \sigma c_2^{2\varepsilon_2}$ is a finite chord.
...
\begin{definition}
\label{def:topBottom}
The substring~$\sigma = \prod_{i' < \ell < j'} \alpha_\ell^{\varepsilon_\ell}$ is \defn{at the bottom} (resp.~\defn{on top}) of the walk~$\omega = \prod_{i < \ell < j} \alpha_\ell^{\varepsilon_\ell}$ if~$i'=-\infty$ or~$\varepsilon_{i'-1} = 1$ and~$j'=+\infty$ or~$\varepsilon_{j'+1} = -1$ (resp.~if~$i'=-\infty$ or~$\varepsilon_{i'-1} = -1$ and~$j'=\infty$ or~$\varepsilon_{j'+1} = 1$).
...
The corresponding perfect matchings (see Remark~\ref{rem:propertiesSurface}) form a cycle with green and red arrows alternating.
Moreover, $\bar Q$ and~$\bar Q\koszul$ have no infinite straight walks.
Since~$|Q_1| = |Q_0|-1$, the corresponding surfaces have $1$ boundary component and genus~$0$.
...
\begin{figure}[h]
	\capstart
	\centerline{\includegraphics[scale=1]{infiniteCurve2.pdf}}
...
\begin{figure}[h]
	\capstart
	\centerline{\includegraphics[scale=1]{infiniteCurve1.pdf}}
...
Here, a crossing is positive when~$\alpha$ leaves a face~$u\dual$ to enter a face~$v\dual$ with the vertex~$u$ being to its right (and thus with~$v$ being to its left).
...
\begin{definition}
\vincent{Mettre la def dans les thm}
