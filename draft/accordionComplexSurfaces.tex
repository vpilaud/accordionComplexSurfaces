...
\begin{proof}
The duality provides a bijection~$v\in (Q_\dissection)_0\mapsto v\dual\in (Q_{\dissection\dual})_0$, and also induces a bijection~${\alpha \in (Q_\dissection)_1 \mapsto \alpha\dual \in (Q_{\dissection\dual})_1}$ with~$s(\alpha\dual) = t(\alpha)\dual$ and~$t(\alpha\dual) = s(\alpha)\dual$.
...
\item Similar statements hold dually for~${{V\dual}\!\!_{\bar Q}}$ and~${\dissection\dual\!\!_{\bar Q}}$, and the notations~$p\dual$ and~$\dualedgeof(v)$ are defined similarly.
...
Let~$\bar Q$ be a locally gentle bound quiver.
For each arc of the dissection~$\dissection_{\bar Q}$ on~$\surface_{\bar Q}$, we fix a point on the interior of this arc, which we call its ``middle point'' (this is the black vertex on the pictures).
...
\begin{proof}
We can assume that~$\gamma$ intersects itself and the arcs of~$\dissection_{\bar Q}$ transversaly and minimally, in the sense that~$\gamma$ does not cross an arc twice in succession in opposite directions (see Definition~\ref{def:crossingCurves}).
...
It would be interesting to find the precise link between~$\NKC[\bar Q\koszul]$ and~$\NKC[\bar Q\koszul]$.
\end{remark}
