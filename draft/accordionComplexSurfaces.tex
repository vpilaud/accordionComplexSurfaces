...
\vincent{Change formulation here to have the pictures on the same line.}
...
Let~$\omega = \ldots \alpha_{i-1}^{\varepsilon_{i-1}}\alpha_{i}^{\varepsilon_i}\alpha_{i+1}^{\varepsilon_{i+1}}$ be a (possibly infinite) walk on~$\bar Q$. 
...
\begin{figure}[h]
	...
	\caption{The order~$\prec_\alpha$ is given by the order in which the curve cross the angle corresponding to~$\alpha$.}
