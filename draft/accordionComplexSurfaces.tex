...
\begin{tikzpicture}[scale=2, thick]
	\node (s) at (-1,0) {$s(\alpha)$};
	\node (t) at (1,0) {$t(\alpha)$};
	\node (v) at (0,1) {$\green v(\alpha)$};
	\node (vd) at (0,-1) {$\red v\dual(\alpha)$};
	\draw[color=black, ->] (s) -> (t) node[midway, above] {$\alpha$};
	\pic [draw, angle radius=.7cm] {angle = s--t--vd};
	\pic [draw, angle radius=.7cm] {angle = vd--s--t};
	\draw[color=green, ->] (v) -> (s) node[midway, above left] {$\green \Enrs{\alpha}$};
	\draw[color=green, ->] (v) -> (t) node[midway, above right] {$\green \Enrt{\alpha}$};
	\draw[color=red, ->] (vd) -> (s) node[midway, below left] {$\red \Ers{\alpha}$};
	\draw[color=red, ->] (vd) -> (t) node[midway, below right] {$\red \Ert{\alpha}$};
\end{tikzpicture}
}
...
This surface is actually endowed with two disjoint sets~$V_{\bar Q}$ and~${V_{\bar Q}}\dual$ of marked points and two dual dissections~$\dissection_{\bar Q}$ and~${\dissection_{\bar Q}}\dual$ defined as follows.
