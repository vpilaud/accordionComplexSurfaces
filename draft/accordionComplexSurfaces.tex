...
\newcommand{\arcof}{\operatorname{\varepsilon}} % arc of a vertex
\newcommand{\dualarcof}{\operatorname{\varepsilon}\dual} % dual arc of a vertex
...
We show that all three simplicial complexes are isomorphic; that they are pure and thin.
...
\pierreguy{Should we talk of g,c,d-vectors in the abstract?}
\vincent{No. I would say that this does not need to be emphasized.}
The aim of this paper is to prove that two combinatorial objects, called the ``non-kissing complex'' and the ``non-crossing complex'', are isomorphic.
...
\vincent{Maybe a sentence here to say that we now review historical / motivation / bibliographic facts.}
...
In the case where the surface is a polygon, the $\tau$-tilting theory of the algebra of a dissection has been studied in \cite{PilaudPlamondonStella}.
\vincent{Should we also mention cluster algebras from surfaces in this paragraph?}
...
We note that this surface is usually different than the one found in \cite{BaurSimoes}.
...
\vincent{Can we delete this notation.}
\end{definition}
...
A \defn{substring} of a walk~$\omega = \prod_{i < \ell < j} \alpha_\ell^{\varepsilon_\ell} \in \walks(\bar Q)$ is a string~$\sigma = \prod_{i' < \ell < j'} \alpha_\ell^{\varepsilon_\ell}$ of~$\strings(\bar Q)$ for some~$i \le i' < j' \le j$, where the inequality~$i \le i'$ (resp.~$j' \le j$) is strict when~$i \ne -\infty$ (resp.~$j \ne \infty$). In other words, $\sigma$ is a factor of~$\omega$ such that
\begin{itemize}
...
\begin{definition}
\label{def:topBottom}
We say that the substring~$\sigma = \prod_{i' < \ell < j'} \alpha_\ell^{\varepsilon_\ell}$ is \defn{at the bottom} (resp.~\defn{on top}) of the walk~$\omega = \prod_{i < \ell < j} \alpha_\ell^{\varepsilon_\ell}$ if~$i'=-\infty$ or~$\varepsilon_{i'-1} = 1$ and~$j'=+\infty$ or~$\varepsilon_{j'+1} = -1$ (resp.~if~$i'=-\infty$ or~$\varepsilon_{i'-1} = -1$ and~$j'=\infty$ or~$\varepsilon_{j'+1} = 1$).
...
We can now define the non-kissing complex of a locally gentle bound quiver following~\cite{McConville, PaluPilaudPlamondon}.
...
The \defn{reduced non-kissing complex}~$\RNKC$ is the simplicial complex whose faces are the collections of pairwise non-kissing bended walks of~$\bar Q$.
\end{definition}
...
\begin{figure}[t]
	...
	\caption{Some pairs~$(\dissection, \dissection\dual)$ of dual cellular dissections on different surfaces. The boundaries of the surfaces are shaded, and the glue symbols indicate how to identify some edges to get the desired surfaces. The first two surfaces are disks, the third is a sphere, the fourth is a cylinder, and the fifth is a punctured torus. The dissection~$\dissection$ is in green while its dual dissection~$\dissection\dual$ is~in~red.}
...
The following definition generalises the one from \cite{MannevillePilaud-accordion} for the case where~$\bar \surface$ is a disk.  It appears in \cite{BaurSimoes} under the name ``permissible arc''.
\pierreguy{Is it ok to cite like this?}
...
\begin{itemize}
\item the edges of~$a$ and~$b$ of~$f$ crossed by~$\alpha$ might coincide, see ...
\item the second condition is automatically satisfied if~$f\dual$ is not a puncture, \ie when it lies on the boundary of~$\surface$.
\end{itemize}
\end{remark}
...
Note that self-crossing slalom never appear in~$\SC$ by definition.
...
Case 1: The arc~$\alpha$ consecutively crosses two arcs~$a$ and~$b$ of~$\dissection$ that do not share a common endpoint.
...
The boundary of this component contains an arc~$c \in \dissection \setminus \{a\}$ that share a common endpoint with~$a$.
Let~$u,v$ be the endpoints of~$c$.
Then~$\alpha$ cuts the cells $u\dual,v\dual$ glued along~$c\dual$ into two connected component, one containing both endpoints of~$\delta$.

Case 2: The arc~$\alpha$ consecutively crosses two arcs~$a$ and~$b$ of~$\dissection$ that share a unique common endpoint, but the region delimited by~$\alpha$,~$a$ and~$b$ is not a disk.
Since the dissection~$\dissection$ is cellular, that region has to contain some marked point~$u \in \dissection\dual$.
One conclude that~$\alpha$ is not a~$\dissection\dual$-slalom similarly as in the first case, by considering the component of~$u\dual \setminus \alpha$ that do not contain~$u$.

Case 3: The arc~$\alpha$ consecutively crosses two arcs~$a$ and~$b$ of~$\dissection$ that share two different endpoints, and none of the two regions delimited by~$\alpha$,~$a$ and~$b$ is a disk.
...
There is a~$\dissection\dual$-arc~$a$ ending in that puncture. When~$\alpha$ crosses~$a$, the puncture stays on the same side of~$\alpha$, hence~$\alpha$ is not a~$\dissection\dual$-slalom.
\end{proof}
...
This construction is directly inspired by the one found in \cite{OpperPlamondonSchroll} (see Remark \ref{rem:comparisonWithOPS} for a comparison of the two constructions).
...
The advantage of Definition~\ref{def:surfaceQuiver} is that it automatically endows with two disjoint sets~$V_{\bar Q}$ and~${V_{\bar Q}}\dual$ of marked points and two dual cellular dissections~$\dissection_{\bar Q}$ and~${\dissection_{\bar Q}}\dual$ defined as follows.
...
\item \label{item:arcs}
      The dissection~$\dissection_{\bar Q}$ has one arc for each vertex~$p \in \bar Q_0$, obtained by concataining the sides~$\Enrt{\alpha} = \Enrs{\beta}$ and~$\Enrt{\alpha'} = \Enrs{\beta'}$ where~$p = t(\alpha) = s(\beta) = t(\alpha') = s(\beta')$, $\alpha\beta \notin I$ and~$\alpha'\beta' \notin I$.
...
\item \label{item:boundary}
      The number~$b$ of boundary components of~$\surface(\bar Q)$ can be computed as follows.
...
\item The genus of the surface~$\surface(\bar Q)$ is
      ...
      Indeed, $2g = 2-\chi(\surface(\bar Q))$, where~$\chi(\surface(\bar Q))$ is the Euler characteristic of~$\surface(\bar Q)$.
      The dissection~$\dissection$ provides a cellular decomposition of~$\surface(\bar Q)$ on which we fill the boundary components and the punctures with disks.  
...
We now apply Remark~\ref{rem:propertiesSurface} to understand the corresponding surfaces.
...
Since~$|Q_1| = |Q_0|-1$, the corresponding surfaces have $1$ boundary component, no puncture and genus~$0$.
...
\vincent{TODO: Describe here the peak and deep facets in terms of the surface.}
\end{example}
...
Then~$\gamma \prec_\alpha \delta$ if~$\gamma$ passes closer (resp.~farther) to~$v$ than~$\delta$ does.
%
\begin{figure}[h]
	\capstart
...
We have thus found a bended walk~$\lambda \in F$ containing an arrow~$\alpha$ or~$\beta$ of~$c$, say~$\alpha$.
...
\para{Geometric proof}
The statement of the lemma translates on~$\surface(\bar Q)$ as follows: for any facet~$F$ of the non-crossing complex~$\NCC$ and any puncture of the dissection~$\dissection_{\bar Q}$, there exists a curve in~$F$ which goes to the puncture.
Assume that there is a puncture with no curves of~$F$ going to it.
Construct a curve~$\gamma$ as follows:~$\gamma$ first unwinds from the puncture, then~$\gamma$ leaves the local picture around the puncture by turning around another marked point (marked~$X$ on the picture below).
...
Firstly, if there are no curves of~$F$ closer to~$X$ than~$\gamma$, then~$\gamma$ can circle around~$X$ until it reaches the boundary (if~$X$ is on the boundary) or infinitely (if~$X$ is a puncture).
Then~$\gamma$ is a~$\dissection_{\bar Q}$-accordion which crosses none of those in~$F$, contradicting the maximality of~$F$.

Secondly, if there is a curve of~$F$ closer to~$X$ than~$\gamma$, then let~$\delta$ be the one furthest from~$X$ ($\delta$ exists by Lemma \ref{lemm:facesHaveMaximalElement}).
...
\vincent{Add a description and a schematic representation of the flip in the accordion complex and in the slalom complex.}
...
\vincent{Orient the flip graph. Connection to the lattice of torsion classes?}
...
\vincent{TODO: exm g- c- and d-vectors}
\end{example}
...
According to Remark~\ref{rem:propertiesSurface}\,(\ref{item:arcs}), each vertex~$p$ of~$Q$ corresponds to an edge~$\arcof(p)$ in the dissection~$\dissection_{\bar Q}$ and  to an edge~$\dualarcof(p) = \arcof(p)\dual$ in the dual dissection~${\dissection_{\bar Q}}\dual$.
...
For a walk~$\omega \in \NKWalks^\pm(\bar Q)$ and a vertex~$p \in Q_0$, the $p$th entry of the $\b{g}$-vector~$\gvector{\omega}$ is given~by
\begin{enumerate}[(i)]
\item The number of times that the $\dissection$-accordion~$\curveof(\omega)$ positively crosses the edge~$\arcof(p)$ of~$\dissection$ minus the number of times it negatively crosses the edge~$\arcof(p)$ of~$\dissection$. Here, the crossing is positive (resp.~negative) if the two angles of~$\dissection$ crossed by~$\curveof(\alpha)$ just before and after~$\arcof(p)$ form a~$\SSS$ (resp.~as a~$\ZZZ$).
\item The number of times that the $\dissection\dual$-slalom~$\curveof(\omega)$ positively crosses the edge~$\dualarcof(p)$ of~$\dissection\dual$ minus the number of times it negatively crosses the edge~$\dualarcof(p)$ of~$\dissection\dual$. Here, the crossing is positive when~$\curveof(\omega)$ leaves a face~$u\dual$ to enter a face~$v\dual$ with the vertex~$u$ being to its right (and thus with~$v$ being to its left), and the crossing is negative otherwise.
\end{enumerate}
\end{proposition}

\begin{proposition}
\label{prop:cVectorsSurface}
Consider a walk~$\omega \in \NKWalks^\pm(\bar Q)$ in a non-kissing facet~$F \in \RNKC$
...
\begin{enumerate}[(i)]
\item the number of crossings between the distinguished subcurve of~$\curveof(\omega)$ in~$\curveof(F)$ and the edge~$\arcof(v)$ of the dissection~$\dissection$,
\item the number of times the distinguished subcurve of~$\curveof(\omega)$ in~$\curveof(F)$ follows the edge~$\dualarcof(v)$ of the dual dissection~$\dissection\dual$. Here, we say that a curve~$\delta$ follows~$\dualarcof(v)$ if~$\dualarcof(v)$ lies on the boundary opposite to~$u$ of a face~$u\dual$ of~$\dissection\dual$ crossed by~$\delta$.
\end{enumerate}
\end{proposition}
...
For a walk~$\omega \in \NKWalks^\pm(\bar Q)$ and a vertex~$p \in Q_0$, the $p$th entry of the $\b{d}$-vector~$\dvector{\omega}$ is the number of crossings of the curve~$\gamma(\omega)$ with the edge of~$X_\deep$ corresponding to~$p$.
\end{proposition}
...
%
%\begin{proof}
%Definitions~\ref{def:cVectorsAccordions} and~\ref{def:gVectorsSlaloms} are immediate translations in terms of curves on the surface~$\surface(\bar Q)$ of Definitions~\ref{def:cVectorsWalks} and~\ref{def:gVectorsWalks} respectively.
%\end{proof}
