...
We conclude this section with four families of examples that illustrate the computation of~$\surface_{\bar Q}$.
...
The set of all such walks forms the \defn{peak facet}~$\set{p_\peak}{p \in Q_0}$ (resp.~the \defn{deep facet}~$\set{p_\deep}{p \in Q_0}$) of the non-kissing complex~$\NKC$.
As an example of Theorem~\ref{thm:complexesCoincide}, let us now describe the corresponding non-crossing facets of~$\NCC[\surface_{\bar Q}]$.

For a vertex~$v \in V$ on the boundary of~$\surface$, we denote by~$\vprevious{v}$ (resp.~$\vnext{v}$) the blossom vertex previous (resp.~next) to~$v$ on the counterclockwise order around the boundary of~$\surface$.
...
\item if~$v$ is a puncture, then rotate around~$v$ in counterclockwise direction (resp.~clockwise direction).
\end{itemize}
...
\end{enumerate}
\end{proposition}
The walk~$\omega'$ of Proposition \ref{prop:flip} is the \defn{flip} of \defn{mutation} of~$F$ at~$\omega$.
The flip is \defn{positive} if~$\sigma$ is on top of~$\omega$, and \defn{negative} otherwise.
...
Then~$F$ forms a~$B$-dissection of the surface, and~$\gamma$ lies in at least one of two configutations~$\ZZZ$ and~$\SSS$ with the other curves of the dissection (where the endpoints may go to infinity if punctures are involved). 
\begin{itemize}
 ...
 \item If the flip is positive, then the~$\SSS$ is turned into a~$\ZZZ$.  If the flip is negative, then the~$\ZZZ$ is turned in to an~$\SSS$.
\end{itemize}
\end{proposition}
\begin{proof}
 Let~$\omega = \walk(\gamma)$.  Using the notation of Proposition \ref{prop:flip}, the local configuration around~$\omega$ (as in Figure \ref{fig:flip}) translates on the (universal cover of the) surface~$\surface(\bar Q)$ as follows.
 \pierreguy{1 picture here.}
...
\begin{figure}[t]
	...
	\caption{A non-kissing facet (left), an accordion facet (middle) and a slalom facet (right) that correspond via the bijections of Theorem~\ref{thm:complexesCoincide} and Proposition~\ref{prop:accordionsSlaloms}. The~$\b{g}$-, $\b{c}$-, and $\b{d}$-matrices of this facet are given in Example~\ref{exm:gcdVectorsWalks}.}
...
We have borrowed \fref{fig:fanAssociahedron} from~\cite{PaluPilaudPlamondon} to illustrate Theorem~\ref{thm:fanAssociahedron}.

\begin{figure}[t]
	\capstart
	\centerline{\includegraphics[scale=.5]{fanAssociahedron1}}
	\centerline{\includegraphics[scale=.5]{fanAssociahedron2}}
	\caption{The $\b{g}$-vector fan~$\gvectorFan$ (left) and the $\bar Q$-associahedron~$\Asso$ (right) of two gentle quivers. Illustration borrowed from~\cite{PaluPilaudPlamondon}.}
