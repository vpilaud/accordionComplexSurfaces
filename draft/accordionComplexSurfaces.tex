...
For a vertex~$v \in V$ on the boundary of~$\surface$, we denote by~$\vprevious{v}$ (resp.~$\vnext{v}$) the blossom vertex previous (resp.~next) to~$v$ on the clockwise order around the boundary of~$\surface$.
...
Let~$F$ be a facet of the non-crossing complex~$\NCC$, and let~$\gamma \in F$.
Then~$F$ forms a~$B$-dissection of the surface, and~$\gamma$ lies in at least one of the two configutations~$\ZZZ$ and~$\SSS$ with the other curves of the dissection (where the endpoints may be punctures). 
\begin{itemize}
 \item 
\end{itemize}
...
For instance, the $\b{g}$-, $\b{c}$-, and $\b{d}$-matrices of the facet illustrated in \fref{} are given by
\[
\gvectors{F} = 
...
\begin{figure}[t]
	...
	\caption{A non-kissing facet (left), an accordion facet (middle) and a slalom facet (right) that correspond via the bijections of Theorem~\ref{thm:complexesCoincide} and Proposition~\ref{prop:accordionsSlaloms}. Their~$\b{g}$-, $\b{c}$-, and $\b{d}$-matrices of this facet are given in Example~\ref{exm:gcdVectorsWalks}.}
...
\vincent{Should we add an example? One of the pictures of~\cite{PaluPilaudPlamondon}?}
