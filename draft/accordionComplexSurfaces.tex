...
\usepackage{enumerate, amsmath, amsfonts, amssymb, amsthm, mathrsfs, wasysym, graphics, graphicx, xcolor, url, hyperref, hypcap,  shuffle, xargs, multicol, overpic, pdflscape, multirow, hvfloat, minibox, accents, array, xifthen, ae, aecompl, blkarray, pifont, mathtools, a4wide}
...
% accordion and zigzag complexes
...
\newcommandx{\ZC}[1][1=\dissection\dual]{\mathcal{K}_{\mathrm{zz}}(#1)} % zigzag complex
...
\begin{abstract}
We consider three combinatorial models for the support $\tau$-tilting complex of a gentle bound quiver: the accordion complex, the zigzag complex, and the non-kissing complex. We unify these models in the same picture.
\end{abstract}
...
\begin{enumerate}
\item Accordion / zigzag / non-kissing / support $\tau$-tilting complexes all coincide for gentle algebras
\item Non-kissing and non-crossing complexes for gentle algebras
\item Combinatorial models for support $\tau$-tilting on gentle algebras: accordion, zigzag and non-kissing complexes
\end{enumerate}
...
\section{Accordion complex, zigzag complex, and non-crossing complex}
...
An \defn{arc} on~$\bar\surface$ is a curve on~$\surface$ connecting two marked points of~$M$ and whose interior is disjoint from~$M$ and the boundary of~$\surface$ (note that arcs homotopic to boundary arcs are allowed here).
As usual, arcs are considered up to homotopy relative to their endpoints in~$\surface \ssm M$.
...
The \defn{faces} of a dissection~$\dissection$ are the connected components of the complement of the union of the arcs of~$\dissection$ in the surface~$\surface$.
We denote by~$\faces(\dissection)$ the set of faces of~$\dissection$.
The dissection~$\dissection$ is \defn{cellular} if all its faces are disks.
\end{definition}

\begin{definition}
Let~$\surface$ be a surface with two sets~$M$ and~$M\dual$ of marked points so that~$M$ and~$M\dual$ alternate along the boundary of~$\surface$.
A dissection~$\dissection$ of~$(\surface, M)$ and a dissection~$\dissection\dual$ of~$(\surface, M\dual)$ are \defn{dual dissections} if there are pairs of mutually inverse bijections~$M\dual \leftrightarrow \faces(\dissection)$ and~$M \leftrightarrow \faces(\dissection\dual)$, denoted~$\dual$ in both directions, such that the dissection~$\dissection$ has an edge joining its vertices~$u,v \in M$ and separating its faces~$f,g \in  \faces(\dissection)$ if and only if~$\dissection\dual$ has an edge joining its vertices~$f\dual, g\dual \in M\dual$ and separating its faces~$u\dual, v\dual \in \faces(\dissection\dual)$.
\end{definition}

Note that contrarily to the usual conventions, the dual vertex of a face~$f$ is not always in the interior of the face.
...
\end{itemize}
In fact, the second point forces the following characterization of the dissections that admit a dual dissection.

\begin{proposition}
\label{prop:conditionsDualDissections}
For a dissection~$\dissection$ of a marked surface~$(\surface, M)$, there exists a set~$M\dual$ of points of~$\surface$ and a dissection~$\dissection\dual$ of~$(\surface, M\dual)$ such that~$\dissection$ and~$\dissection\dual$ are dual dissections if and only if each face of~$\dissection$ has at most one boundary edge.
Moreover, the set~$M\dual$ is unique.
\end{proposition}
...
\begin{example}
Two examples of dual dissections on a surface are represented in \fref{fig:dissections}.
\end{example}
...
\vincent{I am tempted to define here the interior points and blossom points and to only consider arcs passing though these points. Still need to think about it.}
\end{definition}

\subsection{Accordion complex, zigzag complex, and non-crossing complex}

Let~$\surface$ be a surface with two sets~$M$ and~$M\dual$ of marked points and two dual dissections~$\dissection$ and~$\dissection\dual$.
Let~$I$ denote the interior points and $B$ the blossom points.

\begin{definition}
\label{def:accordion}
A \defn{$\dissection$-accordion} is an arc~$\gamma$ of~$(\surface, B)$ such that any two arcs of $\dissection$ crossed consecutively by~$\gamma$
share a marked point with the property that the region bounded by~$\gamma$ and the two arcs around the marked point is homeomorphic to a disk
(in particular, it contains no punctures).
\end{definition}
...
\vincent{Reduced accordion complex.}
\end{definition}

\begin{definition}
\label{def:zigzag}
A \defn{$\dissection\dual$-zigzag} is an arc of~$(\surface, B)$ so that for each face~$f \in \faces(\dissection)$ ...
\end{definition}

\begin{definition}
\label{def:zigzagComplex}
The \defn{$\dissection\dual$-zigzag complex}~$\ZC$ is the simplicial complex of pairwise non-crossing $\dissection$-zigzags.
\vincent{Reduced zigzag complex.}
\end{definition}
...
Similarly, the $\dissection\dual$-zigzags do not depend on~$\dissection$.
\end{remark}

\begin{proposition}
\label{prop:accordionsZigzags}
For two dual dissections~$\dissection$ and~$\dissection\dual$, the $\dissection$-accordions are precisely the $\dissection\dual$-zigzags and the $\dissection$-accordion complex coincides with the $\dissection\dual$-zigzag complex.
\end{proposition}
...
We call \defn{non-crossing complex} of the pair of dual dissections~$(\dissection, \dissection\dual)$ the simplicial complex~$\NCC = \AC = \ZC$.
\end{definition}

\begin{remark}
To clearly visualize Proposition~\ref{prop:accordionsZigzags}, we follow the convention to draw the $\dissection$-accordions (resp.~$\dissection\dual$-zigzags) so that they intersect the arcs of~$\dissection$ (resp.~of~$\dissection\dual$) only at the intersection points of the arcs of~$\dissection$ with the arcs of~$\dissection\dual$.
\vincent{That is interior points}
\end{remark}

\begin{remark}
The $\dissection$-zigzags and the $\dissection\dual$-accordions are defined dually and also coincide.
\end{remark}

\begin{remark}
A quick disclaimer about accordion complexes and zigzag complexes of arbitrary dissections.
As stated in Proposition~\ref{prop:conditionsDualDissections}, a dissection~$\dissection$ with a face containing more than one boundary edge does not admit a dual dissection~$\dissection\dual$.
This prevents to use the definitions given in this section.
Although its accordion complex (resp.~zigzag complex) could still be defined as in~\cite{MannevillePilaud-accordion} (resp.~\cite{GarverMcConville}) for the disk, the resulting complexes would be join of smaller accordion complexes (resp.~zigzag complexes) defined in this paper.
...
Namely, any pair of dual dissections~$(\dissection, \dissection\dual)$ defines a gentle bound quiver~$\bar Q_{\dissection, \dissection\dual}$ such that the non-crossing complex~$\NCC$ is isomorphic to the non-kissing complex~$\NKC[\bar Q_{\dissection, \dissection\dual}]$.
Conversely, any gentle bound quiver~$\bar Q$ gives rise to a surface~$\surface_{\bar Q}$ equipped with a pair of dual dissections~$\dissection_{\bar Q}$ and~$\dissection\dual_{\bar Q}$ such that the non-kissing complex~$\NKC$ is isomorphic to the non-crossing complex~$\NCC[\dissection_{\bar Q}, \dissection\dual_{\bar Q}]$.
In fact, we show that the accordion, zigzag, and non-kissing complexes can all be pictured in a unique way on the surface.
Interestingly, via the correspondence between pairs of dual dissections and gentle bound quivers, the duality between the dissections~$\dissection$ and~$\dissection\dual$ translates to the Koszul duality between the gentle algebras of~$\bar Q$ and~$\bar Q\dual$.
...
For two dual dissections~$\dissection$ and~$\dissection\dual$ of an oriented surface, the quivers~$\bar Q_{\dissection}$ and~$\bar Q_{\dissection\dual}$ are Koszul duals.
...
The surface~$\surface(\bar Q)$ of a gentle bound quiver~$\bar Q = (Q,I)$ is obtained by glueing lozanges around arrows of~$Q$ following the rule given by the relations~$I$. The construction also automatically defines two dual dissections~$\dissection_{\bar Q}$ and~$\dissection\dual_{\bar Q}$ of the surface~$\surface_{\bar Q}$.
\end{definition}
...
\subsection{Accordion, zigzag and non-kissing complexe coincide}
...
\begin{itemize}
\item For any gentle bound quiver~$\bar Q$, the non-kissing complex~$\NKC$ is isomorphic to the non-crossing complex~$\NCC[\dissection_{\bar Q}, \dissection\dual_{\bar Q}]$, therefore to the accordion complex~$\AC[\dissection_{\bar Q}]$ and to the zigzag complex~$\ZC[\dissection\dual_{\bar Q}]$.
\item For any pair of dual dissections~$\dissection, \dissection\dual$ of an oriented surface, the non-crossing complex~$\NCC$ is isomorphic to the non-kissing complex~$\NKC[\bar Q_{\dissection, \dissection\dual}]$.
\end{itemize}
\end{theorem}
