...
In the particular case when the type-cone is simplicial, then we can get rid of the sums.
\begin{corollary}
...
Here, we extend the construction of~\cite[Sect.~3.2]{ArkaniHamedBaiHeYan} to any finite type (simply laced or not) cluster algebra with respect to any seed (acyclic or not) by providing a direct proof that the type-cone of the $\b{g}$-vector fan of any finite type cluster algebra is simplicial.
...
Note that \cref{lem:linearDependencegvectorsCA} implies \cref{thm:generalizedAsso} since it ensures that the type-cone~$\typeCone(\gvectorFan[\B_\circ])$ of the $\b{g}$-vector fan contains the cone of~${\b{h} \in \R^{\principalVariables}}$ such that
\(
...
\subsubsection{Type-cone of finite type $\b{g}$-vector fans}
...
\begin{definition}
Consider a seed~$(\B, \cluster)$ and a cluster variable~$x \in \cluster$. The mutation of~$\seed$ in direction~$x$ is a \defn{mesh mutation} if the entries~$b_{xy}$ for~$y \in \cluster$ are all non-negative, or all non-positive.
\end{definition}

For instance, in the initial seed~$(\B_\circ, \cluster_\circ)$, the mutation in the direction of any variable~${x_\circ \in \cluster_\circ}$ is a mesh.
We call these meshes \defn{initial}.
We denote by~$\meshes$ the set of all pairs~$(x,x')$ where~$x$ and~$x'$ are two non-initial cluster variables of~$\principalClusterAlgebra$ which are exchangeable via a mesh mutation.
The following statement describes the other meshes.

\begin{lemma}
\label{lem:bijectionMeshMutations}
There is precisely one non-initial mesh directed by each positive $\b{c}$-vector of the cluster algebra~$\principalClusterAlgebra$. Therefore~$|\principalVariables| = |\meshes| + n$.
\end{lemma}
...
Consider two adjacent seeds~${(\B, \cluster)}$ and~${(\B', \cluster')}$ with~$\cluster \ssm \{x\} = \cluster' \ssm \{x'\}$ such that the entries~$b_{xy}$ for~$y \in \cluster$ are all non-positive or all non-negative.
...
For any finite type exchange matrix~$\B_\circ$ (acyclic or not), the non-initial mesh mutations precisely correspond to the extremal exchangeable pairs of the $\b{g}$-vector fan~$\gvectorFan[\B_\circ]$.
\end{proposition}
...
\begin{proof}[Sketch of \cref{prop:extremalExchangeablePairsCA} in type~$A$]
...
Our next statement follows from~\cref{prop:extremalExchangeablePairsCA,lem:bijectionMeshMutations}.
...
Combining \cref{coro:simplicialTypeCone,coro:simplicialTypeConeCA,prop:extremalExchangeablePairsCA}, we derive the following description of all polytopal realizations of the $\b{g}$-vector fan~$\gvectorFan[\B_\circ]$.
...
Consider the gentle quivers of \cref{fig:nonkissingFans}.
...
\begin{proof}[Proof of Theorem~\ref{theorem::extricats}]\yann{PG: Est-ce que cela te semble suffisant, ou veux-tu que j'ajoute des d\'etails ? }
