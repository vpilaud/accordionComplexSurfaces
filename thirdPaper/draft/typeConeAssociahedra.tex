...
Consider the gentle quivers of \cref{fig:typeConeNKC}.
\vincent{Todo}
\end{example}
...
In this section, we focus on the following family of gentle quivers, which was also considered in~\cite[Sect.~4]{GarverMcConvilleMousavand}.
...
\begin{proof}[Proof of Theorem~\ref{theorem::extricats}]\yann{PG: Est-ce que cela te semble suffisant, ou veux-tu que j'ajoute des d\'etails ? }
 The proof is know similar to the proof of Theorem~\ref{theo::relations-g-vecteurs}, by using Proposition~\ref{prop::KRextricat} instead of Proposition~\ref{prop::functor-F}, Proposition~\ref{prop::index iso extricat} instead of Corollary~\ref{coro::grothendieck-g-vectors}, and the fact that~$\tc$ is made of projective objects instead of Lemma~\ref{lemm::almost-split}.
\end{proof}
...
We know describe a setup satisfying the assumptions of Section~\ref{sect::setting} that, at the same time, subsumes all examples given so far, while being easily checked in practice.\yann{A reformuler...}
...
\begin{proposition}
Assume that
Then the extriangulated category~$\cat$ satisfies the assumptions (1) to (3) of Section~\ref{sect::setting}.
\end{proposition}
