...
\begin{tikzpicture}[every node/.style={inner sep=6, outer sep=8}]
	...
	\node[rectangle, draw=black, fill=black!10, align=center] (I21) at (-6,-1.5) {\ref{part:geometry}.\ref{subsec:typeConeAsso} -- Toy example \\ Classical associahedra};
	\node[rectangle, draw=orange, fill=orange!10, align=center, text width=5cm] (I22) at (0,-1.5) {\ref{part:geometry}.\ref{subsec:typeConeCA} -- Cluster algebras \\ \& generalized associahedra};
	\node[rectangle, draw=green, fill=green!10, align=center, text width=5cm] (I23) at (6,-1.5) {\ref{part:geometry}.\ref{subsec:typeConeNKC} -- Gentle algebras \\ \& generalized accordiohedra};
% 	\node[rectangle, draw=red, fill=red!10, align=center, text width=5.4cm] (I24) at (7,0) {\ref{part:geometry}.\ref{subsec:typeConeGA} -- Graphical nested complexes \\ \& graph associahedra};
	...
	\draw[decorate, decoration={shape backgrounds, shape=dart, shape size=2mm, shape evenly spread=8}, draw=blue, fill=blue!10] (I1.180) to [bend right, looseness=1.1] node[label={[shift={(-1.6,-1.5)}]applied to}]{} (I21.90);
	\draw[decorate, decoration={shape backgrounds, shape=dart, shape size=2mm, shape evenly spread=3}, draw=blue, fill=blue!10] (I1.270) to node[label={[shift={(1,-1.)}]applied to}]{} (I22.90);
	\draw[decorate, decoration={shape backgrounds, shape=dart, shape size=2mm, shape evenly spread=8}, draw=blue, fill=blue!10] (I1.360) to [bend left, looseness=1.1] node[label={[shift={(1.6,-1.5)}]applied to}]{} (I23.90);
...
We still consider an essential complete simplicial fan~$\Fan$ in~$\R^n$, the $N \times n$-matrix~$\b{G}$ whose rows are the rays of~$\Fan$, and an $(N-n) \times N$-matrix~$\b{K}$ which spans the left kernel of~$\b{G}$ (\ie $\b{K}\b{G} = 0$).
...
\arnau{Vincent, can you please check the coordinates? C.\ Stump thought that two facets had some typo (a $\pm 1$ was in the wrong column)}
