...
We know from~\cite{} that the indecomposable objects of~$K^{[-1,0]}(\proj \Lambda)$ are in natural bijection with the indecomposable~$\Lambda$-modules and with the shifted indecomposable projective~$\Lambda$-modules.  Thus it has finitely many isomorphism classes of indecomposable objects if and only if~$\Lambda$ is representation-finite.  This gives a setting in which to apply Theorem~\ref{}.
