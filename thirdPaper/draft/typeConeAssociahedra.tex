...
\newcommand{\simpleRoot}{\b{\alpha}} % simple root
\newcommand{\fundamentalWeight}{\b{\omega}} % fundamental weights
...
\begin{tikzpicture}[every node/.style={inner sep=6, outer sep=8}]
	...
	\node[ellipse, draw=orange, fill=orange!10, align=center, text width=5cm, inner sep=2, minimum height=1cm] (II1) at (-4.5,-4.2) {\ref{part:algebra}.\ref{sec:clusterCategories} -- Cluster categories};
	\node[ellipse, draw=green, fill=green!10, align=center, text width=5cm, inner sep=0, minimum height=1cm] (II2) at (4.5,-4.2) {\ref{part:algebra}.\ref{sec:extricats} -- Extriangulated categories};
...
\end{itemize}
A fan is \defn{simplicial} if all its cones are, \defn{complete} if the union of its cones covers the ambient space~$\R^n$, and \defn{essential} if it contains the cone~$\{\b{0}\}$. Note that every complete fan is the product of an essential fan with its lineality space.
...
Let~$\HH \eqdef \bigset{\b{x} \in \R^{n+1}}{\sum_{\ell \in [n+1]} \b{x}_\ell = 0}$ and~$\pi : \R^{n+1} \to \HH$ denote the orthogonal projection.
...
Principal coefficients cluster algebras are $\Z^n$-graded (in the fundamental weight basis~$\{\simpleWeight_x\}_{x \in \cluster_\circ}$ of~$V$) for the degree function~$\deg(\B_\circ,\cdot)$ on $\principalClusterAlgebra$ obtained by setting
\(
