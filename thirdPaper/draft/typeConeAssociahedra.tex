...
\newcommand{\gvector}[1]{\b{g}(#1)} % g-vector of the path #1
\newcommand{\gvectors}[1]{\b{g}(#1)} % g-vectors of the paths #1
...
Conversely, we know from \cref{rem:dimTypeCone} that there are at least~$|\walks|-|Q_0|$ extremal exchangeable pairs. Since this~$|\strings| = |\distinguishableStrings| = |\walks| - |Q_0|$ by~\cref{prop:bijectionStringsWalks}, we conclude that all exchangeable pairs~$\{\cc{\sigma}, \hh{\sigma}\}$ for~$\sigma \in \strings$ are extremal.
\end{proof}

Combining \cref{coro:simplicialTypeCone}, \cref{prop:bijectionStringsWalks} and \cref{prop:extremalExchangeablePairsNKC}, we derive the following description of all polytopal realizations of the non-kissing fan.

\begin{corollary}
For any gentle quiver~$\quiver$ with no self-kissing walk and any~$\b{\ell} \in \R_{>0}^{\strings}$, the polytope
\[
R_\b{\ell}(\quiver) \eqdef \bigset{\b{z} \in \R^{\walks}}{\b{z} \ge 0 \text{ and } \b{z}_{\cc{\sigma}} + \b{z}_{\hh{\sigma}} - \b{z}_{\hc{\sigma}} - \b{z}_{\ch{\sigma}} = \ell_\sigma \text{ for all } \sigma \in \strings}
\]
...
\]
\begin{proposition}[]\pg{Add references to BMR and KR}
...
\begin{proposition}[]\pg{Cite Palu}
...
\begin{remark}
 Although the field~$\field$ is assumed to be algebraically closed in~\cite{} \pg{Cite Palu}, this assumption is not needed in the proof, and the result is valid over any field.
\end{remark}
...
\end{definition}
In particular,~$\Ksp(\cat)$ is isomorphic to a free abelian group over the set~$\ind(\cat)$.  The choice of the notation for~$\Kg(\cat)$ is motivated by the next section \pg{Add ref}, where we study relations between~$\b g$-vectors.
...
\begin{theorem}\label{theo::relations-g-vecteurs}
 Let~$\cat$ be a category satisying the hypotheses of \ref{} \pg{add ref}.  Then~$\cat$ has only finitely many isomorphism classes of indecomposable objects if and only if the set~\[L:=\big\{\ell_X \ | \ X\in \ind(\cat)\setminus \add(\susp T) \big\}\] generates the kernel of~$\b g:\Ksp(\cat) \to \Kg(\cat)$. 
 In this case, the set~$L$ is a basis of the kernel of~$\b g$, and \pg{add formula}, for any $x\in \ker(\b g)$, we have that
 \[
...
\begin{proof}
 ...
 \]
 Applying now the functor~$\Hom{\Lambda}(-, FB)$, we get an exact sequence
 \[
  0 \xrightarrow{} \Hom{\Lambda}(F\susp^{-1} A, FB) \xrightarrow{} \Hom{\Lambda}(FE, FB) \xrightarrow{Ff^*} \Hom{\Lambda}(FA, FB) \xrightarrow{} \coker(Ff^*) \xrightarrow{} 0
 \]
 If~$B\ncong A$, then the definition of an almost-split triangle implies that~$Ff^*$ is surjective. \pg{add an exact sequence here; image of Ffstar is non-invertible elements} Thus~$\coker(Ff^*) = 0$, and by additivity of the dimension in exact sequences, we get that~$\langle \ell_A, B \rangle =0$.
 
 If~$B\cong A$, then~$\coker(Ff^*)$ is isomorphic to the residue field of~$\End{\Lambda}(FA)$, which is isomorphic to the residue field~$\field_A$ of~$A$.
\end{proof}
...
%%%%
\subsection{Application to~$\b g$-vectors}

We recall the following results and definition from \cite{} \pg{cite DK and Palu}.
...
The notion of index is very close to the definition of the map~$\b g:\Ksp(\cat)\to \Kg(\cat)$ of Definition~\ref{} \pg{ref}.  The link is given by the following result.
...
\begin{proposition}
 ...
 \[
  X\xrightarrow{} E \xrightarrow{} Y \xrightarrow{h} \susp X \quad \textrm{and} \quad Y\xrightarrow{} E' \xrightarrow{} X \xrightarrow{h'} \susp Y
 \]
 be triangles with~$h$ and~$h'$ non-zero (these triangles are unique up to isomorphism).  Then~$\CC(Y)$ is obtained by performing a positive mutation on~$\CC(X)$ in some seed if and only if~$h\in (\susp T)$.
 \begin{proof}
