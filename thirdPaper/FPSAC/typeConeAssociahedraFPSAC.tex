...
\resume{
Nous \'etudions le type cone (\ie l'espace de toutes les r\'ealisations polytopales) des \'eventails de $\b{g}$-vecteurs de complexes de type amassés (complexes amassés de type fini, complexes platoniques d'alg\`ebres aimables, et complexes emboit\'es graphi\-ques). Nous montrons que ce cone est souvent simplicial, ce qui explique une \'el\'egante construction ``cin\'ematique'' de l'associa\`edre comme section d'un orthant positif de haute dimension par certains sous-espaces affines param\`etrés par un orthant positif de basse dimension.
}
...
\item the nested fan is the normal fan of a polytope~$\Asso[\graphG]$, called \defn{graph associahedron}, constructed in~\cite{CarrDevadoss, FeichtnerSturmfels, Postnikov, Zelevinsky}.
